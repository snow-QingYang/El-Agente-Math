...
def block_fragment(block):
    ...
    if block["type"] == "table":
        ...
        return (caption + "\n" + body).strip()
    if block["type"] == "image":
        return f"![]({block['img_path']})"
    return ""


def find_in_md(md_text: str, fragment: str):
    fragment = fragment.strip()
    if not fragment:
        return None
    pattern = re.sub(r"\\s+", r"\\s+", re.escape(fragment))
...
def pick_anchor_block(primary, fallbacks):
    if primary and block_has_text(primary):
        return primary
    for block in fallbacks:
        if block and block_has_text(block):
            return block
    return None
...
def main():
    parser = argparse.ArgumentParser(
        description="Locate a PDF text block and map it to MinerU outputs."
    )
    parser.add_argument(
        "--id",
        required=True,
        help="MinerU output folder name, e.g. 1467_PriViT_Vision_Transformer",
    )
    parser.add_argument(
        "--line",
        required=True,
        help="Line text to locate in the PDF (line number content).",
    )
    parser.add_argument(
        "--out",
        default=None,
        help="Output markdown path (default: <id>/matched.md).",
    )
    args = parser.parse_args()
    start_line, end_line = parse_line_spec(args.line)
    if not start_line:
        raise ValueError(f"Invalid line spec: {args.line}")

    base_root = Path(args.id)
    if base_root.is_absolute():
        base_dir = base_root / "input" / "auto"
    elif len(base_root.parts) > 1:
        base_dir = base_root.resolve() / "input" / "auto"
    else:
        base_dir = Path(__file__).resolve().parent / args.id / "input" / "auto"
    pdf_path = base_dir / "input_origin.pdf"
    content_list_path = base_dir / "input_content_list.json"
    md_path = base_dir / "input.md"
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        if range_loc:
            ...
            if args.out:
                ...
                output_path = base_dir.parent.parent / "matched.md"

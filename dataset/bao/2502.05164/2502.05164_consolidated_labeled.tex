...
\subsection{Setup}
Each task corresponds to a distribution $D$ over the probability distribution of data: <<FORMULA_0004>>.
Let <<FORMULA_0005>>, define the sampling of the tokens. Let the noise corruption be defined by <<FORMULA_0006>>. The random sequence <<FORMULA_0007>> are given as ``context" (input) to a sequence model <<FORMULA_0008>> which outputs an estimate <<FORMULA_0009>> of the original <<FORMULA_0010>>-th token . The task is to minimize the expected loss <<FORMULA_0011>> for some loss function <<FORMULA_0012>>. Namely, our problem is to find
<<FORMULA_0013>>

In practice, we choose <<FORMULA_0014>>, a pure token corrupted by the addition of isotropic Gaussian noise <<FORMULA_0015>>, and our objective function to minimize is the mean squared error (MSE) <<FORMULA_0016>>. 

In the following subsection, we explain the pure token distributions for three specific tasks. These tasks are of course structured so that a one-layer transformer has the expressivity to capture a solution, which, as <<FORMULA_0017>>, provides an optimal solution, in some sense. To that end, we derive Bayes optimal estimators for each of the three tasks, under the assumption that we know the original distribution $p_X$ of pure tokens. In Section \ref{sec:experiments}, we use these estimators as baselines to evaluate the performance of the denoiser <<FORMULA_0018>> based on a one-layer transformer trained on finite datasets. 
...
Below we describe the task-specific distributions $p_X$ and the process for sampling tokens <<FORMULA_0020>>. 
The same corruption process applies to all cases: <<FORMULA_0021>>. 
...
\begin{itemize}
  \item Let $P$ be the orthogonal projection operator to a random $d$-dim subspace $S$ of $\mathbb{R}^n$, sampled according to the uniform measure, induced by the Haar measure on the coset space <<FORMULA_0022>>, on the Grassmanian <<FORMULA_0023>>, the manifold of all $d$-dimensional subspaces of $\mathbb{R}^n$. 
  %\bluebf{note: reversed $G$ notation}
  % swapped notation: G(n,d) -> G(d, n)
  % Thanks!-A
  %
  \item Let <<FORMULA_0024>> and define <<FORMULA_0025>>; we use this procedure to construct the starting sequences <<FORMULA_0026>> of <<FORMULA_0027>> independent tokens.
\end{itemize}
We thus have <<FORMULA_0028>>, with the Haar distribution of $P$ characterizing the task ensemble associated with $D$. 
...
\begin{itemize}
  \item Choose a random <<FORMULA_0029>>-dimensional subspace $V$ of $\mathbb{R}^n$, sampled according to the uniform measure, as before, on the Grassmanian <<FORMULA_0030>>.
  ...
  Inside $V$, sample uniformly from the radius $R$ sphere (once more, a Haar induced measure on a coset space <<FORMULA_0031>>).
  We use this procedure to construct input sequences <<FORMULA_0032>> of <<FORMULA_0033>> independent tokens.
\end{itemize}

In practice, we uniformly sample points with fixed norm in $\mathbb{R}^d$ and embed them in $\mathbb{R}^n$ by concatenating zeros. We then rotate the points by selecting a random orthogonal matrix <<FORMULA_0034>>. 
...
<<FORMULA_0035>>. The density is

<<FORMULA_0036>>

where <<FORMULA_0037>> are normalizing constants. 
%We choose $\sigma_\alpha=\sigma, \:\,\forall\: \alpha$. 
The $\mu_a$ are independently chosen from a uniform distribution on the radius $R$ sphere of dimension <<FORMULA_0038>>, centered around zero. The distribution of tasks $D$, is decided by the choice of <<FORMULA_0039>>.
...
As is well-known, the Bayes optimal predictor of a quantity is given by the posterior mean. We use that fact to compute the Bayes optimal loss.
 ...
 In particular, we seek a function <<FORMULA_0041>> such that <<FORMULA_0042>> is minimized. 
...
Since the perturbation~$Z$ is Gaussian, the posterior distribution of $X$, given <<FORMULA_0043>> is
<<FORMULA_0044>>
 where <<FORMULA_0045>> is a normalizing factor (see Appendix \ref{appendix:Bayes-notation} for more explanation). 
...
% %\end{aligned}
% \end{equation*}
The following proposition sets up a baseline to which we expect to compare our results as <<FORMULA_0046>>. The proof is in Appendix \ref{appendix:performance-bound}.
\begin{prop}
...
This lower bound is met when <<FORMULA_0048>>. 
\end{prop}

Thus, the Bayes optimal denoiser is the posterior expectation for $X$ given <<FORMULA_0049>>. The expected loss is found by computing the posterior sum of variances. 
...
In the nonlinear manifold denoising problem, we focus on the case of lower dimensional spheres $S$ (e.g. the circle <<FORMULA_0052>>). 
For such manifolds, the Bayes optimal answer is given by the following proposition.

\begin{prop}
\label{prop:Bayes-optimal-manifold}
For $p_X$ defined as in Subsection \ref{case2}, with $P$ being the orthogonal projection operator to $V$, the <<FORMULA_0053>> dimensional linear subspace, with $R$ being the radius of sphere $S$, the Bayes optimal answer is
<<FORMULA_0054>>
where <<FORMULA_0055>> and <<FORMULA_0056>> is the modified Bessel function of the first kind.
\end{prop}


\paragraph{Clustering case.}
For clustering with isotropic Gaussian mixtures 
<<FORMULA_0057>>, the Bayes optimal predictors for some important special cases are as follows. See Appendix \ref{appendix:bayes-case3} for the general case.
\begin{prop}
\label{prop:bayes-case3}
For general isotropic Gaussian model with <<FORMULA_0058>> for all <<FORMULA_0059>>.
 %\begin{equation}
...
\textbf{Input:}
Let <<FORMULA_0063>>, be distributions sampled for one of the tasks. For each distribution $p_X^{(i)}$, we sample <<FORMULA_0064>> taking value in $\mathbb{R}^{n \times (L+1)}$ be an input to a sequence model. We also retain the true <<FORMULA_0065>>-th token $X_{L+1}^{(i)}$ for each $i$.  

\textbf{Objective:}
Given an input sequence $E^{(i)}$, return the uncorrupted final token $X_{L+1}^{(i)}$. We consider the mean-squared error loss over a collection of $N$ training pairs, <<FORMULA_0066>>, 
<<FORMULA_0067>>
where <<FORMULA_0068>> denotes the parametrized function predicting the target final token based on input sequence $E^{(i)}$.
...
There we have <<FORMULA_0069>>. Note that, by the strong law of large numbers, <<FORMULA_0070>> is a random matrix that almost surely converges component-by-component to the orthogonal projection $P$ as <<FORMULA_0071>>, since, for each $t$, <<FORMULA_0072>> has the expectation $\sigma_0^2P$ and that $X_t$ is a Gaussian random variable with zero mean and a finite covariance matrix. So we could propose
...
\caption{
  ...
  All cases use a context length of <<FORMULA_0074>>, ambient dimension <<FORMULA_0075>>, and are trained with Adam on a dataset of size 800 with batch size 80 and standard weight initialization <<FORMULA_0076>>. 
...
where <<FORMULA_0078>> taking values in $\R^{n\times L}$, 
with learnable weights <<FORMULA_0079>> abbreviated by $\theta$.
Note that, when <<FORMULA_0080>>, and 
<<FORMULA_0081>>, <<FORMULA_0082>> should approximate the Bayes optimal answer <<FORMULA_0083>> as <<FORMULA_0084>>. For a detailed discussion of the convergence rate, see Appendix~\ref{appendix:convergence-rates}, in general, and Proposition~\ref{prop:linear-case-rate}, in particular.

Similarly, we could argue that the second two problems, the $d$-dimesional spheres and the <<FORMULA_0085>> zero limit of the Gaussian mixtures could be addressed by  softmax attention
<<FORMULA_0086>>
taking values in $\mathbb{R}^{n }$. 
% where 
...
The function <<FORMULA_0087>> is applied column-wise. 

% We then define 
% \begin{equation}
%   F(E,\theta):=[\textrm{Attn}(E,W_{PV},W_{KQ})]_{:,L+1}.    
% \end{equation}
For both problems, namely the spheres and the <<FORMULA_0088>> Gaussian mixtures, we could have <<FORMULA_0089>> with <<FORMULA_0090>> providing Bayes optimal answers as <<FORMULA_0091>>. 
...
<<FORMULA_0092>>
converges almost surely to the Bayes optimal answer <<FORMULA_0093>> for all <<FORMULA_0094>>, as <<FORMULA_0095>>. The optimal parameter $\theta^*$ refers to <<FORMULA_0096>>.
\end{theorem}
...
As <<FORMULA_0098>>,
<<FORMULA_0099>>
where <<FORMULA_0100>> is the empirical mean. 
\end{proposition}
...
For case 1, note that <<FORMULA_0101>> and covariance of $X_t$ is finite, <<FORMULA_0102>>, and <<FORMULA_0103>>, allowing us to drop <<FORMULA_0104>> as <<FORMULA_0105>>. If, in addition, $\epsilon$ is small, only the second term survives. Thus, <<FORMULA_0106>> starts to approximate <<FORMULA_0107>> when $L$ is large and $\epsilon$ is small, with <<FORMULA_0108>> large. 
...
\caption{
  (a) Trained linear attention network converges to Bayes optimal estimator as context length increases (<<FORMULA_0109>>, <<FORMULA_0110>>, <<FORMULA_0111>>). 
  (b) A network trained to denoise subspaces of dimension <<FORMULA_0112>> can accurately denoise subspaces of different dimensions presented at inference time, given sufficient context.
}
...
<<FORMULA_0113>>. 
Fig. \ref{fig:empirical-training-triple} shows that a one-layer network of size <<FORMULA_0114>> trained on tasks with <<FORMULA_0115>> indeed achieves this bound, training to nearly diagonal weights with the appropriate scale
%$\left\langle w_{KQ}^{(ii)} \right\rangle \left\langle w_{PV}^{(ii)} \right\rangle = 0.327 \approx \tfrac{1}{3}$. 
<<FORMULA_0116>> (similar weights are learned for each seed, up to a sign flip). 

Fig. \ref{fig:empirical-ICL}(a) displays how this bound is approached as the context length $L$ of training samples is increased. In Fig. \ref{fig:empirical-ICL}(b) we study how the performance of a model trained to denoise random subspaces of dimension <<FORMULA_0117>> is affected by shifts in the subspace dimension at inference time. We find that when provided sufficient context, such models can adapt with mild performance loss to solve more challenging tasks not present in the training set.

It is evident from Fig. \ref{fig:empirical-training-triple}(a) that the softmax network performs similarly to the linear one for this task. We can understand this through the small argument expansion of the softmax function mentioned above. The learned weights displayed in Fig. \ref{fig:empirical-training-triple}(b) indicate that <<FORMULA_0118>> becomes small (note it decreases by a factor <<FORMULA_0119>> relative to $\beta^\textrm{linear}$), 
while the value scale <<FORMULA_0120>> becomes larger by a similar factor <<FORMULA_0121>> to compensate.
...
Moreover, we find that when the entire prompt undergoes a global invertible transformation <<FORMULA_0122>>, the optimal attention weights are no longer scaled identity matrices but acquire a structured form determined by $A$. Both linear and softmax attention layers are able to recover this structure through training; see Appendix \ref{appendix:sec-coord-transform} for details and empirical verification.

\subsection{Case 2 -- Nonlinear manifolds}
%
Fig. \ref{fig:empirical-training-triple} (case 2) shows networks of size <<FORMULA_0123>> trained to denoise subspheres of dimension <<FORMULA_0124>> and radius <<FORMULA_0125>>, with corruption <<FORMULA_0126>> and context length <<FORMULA_0127>>. 
Once again, the network trains to have scaled identity weights. 

We note that although the network nearly achieves the optimal MSE on the test set, the weights appear at first glance to deviate slightly from the Bayes optimal predictor of Section \ref{sec:bayes-optimal-predictors}, which indicated <<FORMULA_0128>>, <<FORMULA_0129>> with <<FORMULA_0130>>. To better understand this, we consider a coarse-grained MSE loss landscape by scanning over $\alpha$ and $\beta$. See Fig. \ref{fig:loss-landscape-MSE-2d}(a) in Appendix \ref{appendix:loss-landscapes-and-extra-training}. We find that the 2D loss landscape has roughly hyperbolic level sets which is suggestive of the linear attention limit, where the weight scales become constrained by their product <<FORMULA_0131>>. Reflecting the symmetry of the problem, we also note mirrored negative solutions  (i.e. one could also identify <<FORMULA_0132>>, <<FORMULA_0133>> from the analysis in Section \ref{sec:bayes-optimal-predictors}). Importantly, the plot shows that the trained network lies in the same valley of the loss landscape as the optimal predictor, in agreement with Fig. \ref{fig:empirical-training-triple}. Moreover, the shape of the loss landscape suggested that linear attention might also be applicable to this case, which we demonstrate and discuss further in Appendix \ref{appendix:loss-landscapes-and-extra-training}. %Fig. \ref{fig:case2and3-linear-vs-softmax}.

\subsection{Case 3 -- Gaussian mixtures}

Figure \ref{fig:empirical-training-triple} (case 3) shows networks of size <<FORMULA_0134>> trained to denoise balanced Gaussian mixtures with <<FORMULA_0135>> components that have isotropic variance <<FORMULA_0136>> and centers randomly placed on the unit sphere in $\mathbb{R}^n$. The corruption magnitude is <<FORMULA_0137>> and context length is <<FORMULA_0138>>. The baselines show the zero predictor (dashed grey line) as well as the optimum from Proposition (\ref{prop:bayes-case3}) (pink) and its <<FORMULA_0139>> approximation Eq. (\ref{eq:bayes-case3-zerolimit}) (grey). 

The trained weights qualitatively approach the optimal estimator for the zero-variance limit but with a slightly different scaling: while the scale of $W_{PV}$ is <<FORMULA_0140>>, the $W_{KQ}$ scale is <<FORMULA_0141>>. To study this, we provide a corresponding plot of the 2D loss landscape in Fig. \ref{fig:loss-landscape-MSE-2d}(a) in Appendix \ref{appendix:loss-landscapes-and-extra-training}. While the symmetry of the previous case has been broken (the context cluster centers <<FORMULA_0142>> will not satisfy <<FORMULA_0143>>), we again find that the trained network lies in the anticipated global valley of the MSE loss landscape. 
...
In each of the denoising problems studied above, we have shown analytically and empirically that the optimal weights of the one-layer transformer are scaled identity matrices <<FORMULA_0144>>. In the softmax case, the trained denoiser can be concisely expressed as 
...
<<FORMULA_0145>>
re-written such that <<FORMULA_0146>> stores pure context tokens.
...
\caption{
  Gradient descent denoising for the nonlinear manifold case (spheres) in <<FORMULA_0148>> with <<FORMULA_0149>>. A context-aware dense associative memory network <<FORMULA_0150>> is constructed whose gradient corresponds to the Bayes optimal update (trained attention layer). Note that the density of sampled context tokens sculpts the valleys of the energy landscape. 
  ...
  Right: Iterating the denoising process\textemdash as is conventional for Hopfield networks\textemdash can potentially degrade the estimate by causing it to become query-independent (e.g. converging to a distant minimum). Here <<FORMULA_0151>> and <<FORMULA_0152>>. 
  }
...
Gradient descent iteration <<FORMULA_0153>> yields

<<FORMULA_0154>> 


It is now clear that initializing the state to the query <<FORMULA_0155>> and taking a single step with size <<FORMULA_0156>> recovers the behavior of the trained attention model (Fig. \ref{fig:energy-landscape-denoising}). 
...
Intriguingly, the self-attention heads of trained transformers sometimes exhibit weights $W_{KQ}$, $W_{PV}$ that resemble scaled identity matrices, i.e. <<FORMULA_0157>> with small fluctuations <<FORMULA_0158>>,
...
\begin{itemize}
  ...
  \item <<FORMULA_0159>> -- the value of the $t$-th random input token.
  \item <<FORMULA_0160>> -- the random variable input to the sequence model. The ``tilde" indicates that the final token has in some way been corrupted. $E$ takes values <<FORMULA_0161>>. Note: while capital $X$ or $X_i$ here denotes a random variable, in Section \ref{appendix:attention-and-softmax} use $X_{1:L}$ or simply $X$ to refer to the realized matrix of input tokens.
...
\begin{itemize}
  \item <<FORMULA_0162>> is task-dependent (the three scenarios considered here are introduced above).
  
  \item <<FORMULA_0163>> where <<FORMULA_0164>>. 
  For a sum of independent random variables, <<FORMULA_0165>>, their pdf is a convolution <<FORMULA_0166>>. Thus:
    <<FORMULA_0167>>
   where <<FORMULA_0168>> is a constant. 
  ...
  \item <<FORMULA_0171>>: By Bayes' theorem, this is
...
Note the final line is independent of $f$. This inequality becomes an equality when <<FORMULA_0175>>. 
\end{proof}
...
<<FORMULA_0176>>
where <<FORMULA_0177>> is a normalizing factor. The noise can be decomposed into parallel and perpendicular parts using the projection $P$ onto $S$, i.e.
<<FORMULA_0178>>
so that 
<<FORMULA_0179>>

Only the first factor matters for <<FORMULA_0180>> since it depends on $x$. Then, for <<FORMULA_0181>>, the linear subspace supporting $p_X$,  dropping the $x$ independent <<FORMULA_0182>> contribution,   \\
<<FORMULA_0183>>

Thus, <<FORMULA_0184>>.

Using <<FORMULA_0185>>, <<FORMULA_0186>>, and the independence of $X$ and $Z$
<<FORMULA_0187>>
\end{proof}
...
\begin{proof}
In the nonlinear manifold denoising problem, we focus on the case of lower dimensional spheres $S$ (e.g. the circle <<FORMULA_0188>>). 
For such manifolds, we have
<<FORMULA_0189>>
We have used the fact that <<FORMULA_0190>> and that <<FORMULA_0191>> is fixed on the sphere.

The integrals can be evaluated directly once the parameters are specified. If $S$ is a $d$--sphere of radius $R$, then the optimal predictor is again a shrunk projection of <<FORMULA_0192>> onto $S$,

<<FORMULA_0193>>
where we used identities involving  <<FORMULA_0194>>, modified Bessel function of the first kind of order $\nu$ \cite{gradstein2007zwillinger}. The vector 
<<FORMULA_0195>>
...
Doing the integrals and using the expressions for <<FORMULA_0202>>
<<FORMULA_0203>>


In the case that the center norms <<FORMULA_0204>> are independent of $a$ and variances <<FORMULA_0205>>, we have
<<FORMULA_0206>>


Note that in the limit that <<FORMULA_0207>> , this becomes expressible by one-layer self-attention, since one can simply replace the matrix of cluster centers <<FORMULA_0208>> implicit in the expression with the context $X_{1:L}$ itself, 
...
Given a sequence of $L_{\text{seq}}$ input tokens <<FORMULA_0210>> represented as a matrix <<FORMULA_0211>>, standard self-attention defines query, key, and value matrices
%K = W_K X$, $Q = W_Q X$, $V = W_V X$  
<<FORMULA_0212>>
where <<FORMULA_0213>> and <<FORMULA_0214>>. 
The softmax self-attention map \cite{Vaswani2017} is then 
%$f(X)=X + V \textrm{softmax}(QK^T)$.
<<FORMULA_0215>>


  On merging $W_K$, $W_Q$ into <<FORMULA_0216>>: 
  The simplification <<FORMULA_0217>> (made here and elsewhere) is general only when <<FORMULA_0218>>; in that case, the product $W_{KQ}$ can have rank $n$ and thus it is reasonable to work with the combined matrix. 
  On the other hand, if <<FORMULA_0219>>, then the rank of their product is at most <<FORMULA_0220>> and thus there are matrices in $\mathbb{R}^{n \times n}$ that cannot be expressed as <<FORMULA_0221>>. A similar point can be made about $W_{PV}$. 
  We note that while <<FORMULA_0222>> may be used in practical settings, one often also uses multiple heads which when concatenated could be (roughly) viewed as a single higher-rank head. 
...
\label{appendix:self-attention-denoising}
 ...
 We now consider a simplified one-layer linear transformer in term of our variable <<FORMULA_0224>> taking values in $\R^{n\times (L+1)}$ and start with the linear transformer which still has sufficient expressive power to capture our finite sample approximation to the Bayes optimal answer in the linear case.
Inspired by \citet{bartlett2024jmlr}, we define 
<<FORMULA_0225>>
taking values in $\mathbb{R}^{n \times (L+1)}$. The additional aspect compared to the last subsection is the masking matrix  <<FORMULA_0226>> which is of the form
<<FORMULA_0227>>
preventing  <<FORMULA_0228>> from being added to the output. 

Note that this more detailed expression is equivalent to the form used in the main text. 
<<FORMULA_0229>>
% where $E_{0} = E_{:,1:L}, \tilde X = E_{:, L+1}$.

With learnable weights <<FORMULA_0230>> abbreviated by $\theta$, we define 
<<FORMULA_0231>>
Note that, when <<FORMULA_0232>>, and 
<<FORMULA_0233>>, <<FORMULA_0234>> should approximate the Bayes optimal answer <<FORMULA_0235>> as <<FORMULA_0236>>.

Similarly, we could argue that the second two problems, the $d$-dimesional spheres and the <<FORMULA_0237>> zero limit of the Gaussian mixtures could be addressed by the full softmax attention
<<FORMULA_0238>>
taking values in $\mathbb{R}^{n \times (L+1)}$ where 
<<FORMULA_0239>> is a masking matrix  of the form
<<FORMULA_0240>>
once more, preventing the contribution of <<FORMULA_0241>> value to the output.  
The function <<FORMULA_0242>> is applied column-wise. 
...
<<FORMULA_0245>>
Both the numerator <<FORMULA_0246>>  and the denominator <<FORMULA_0247>> are averages of independent and identically distributed bounded random variables. By the strong law of large numbers, as <<FORMULA_0248>>, the average vector in the numerator converges to almost surely to <<FORMULA_0249>> for each component, while the average in the denominator almost surely converges <<FORMULA_0250>>, which is positive. So, as <<FORMULA_0251>>, the ratio in Eq. \ref{eq:softmax-ratio} converges almost surely to 
 <<FORMULA_0252>>
which is the Bayes optimal answer <<FORMULA_0253>> for all <<FORMULA_0254>>.
\end{proof}


\section{Further discussion of convergence rates as <<FORMULA_0255>> and the dependence on dimensions}
\label{appendix:convergence-rates}

Our analysis primarily focused on the asymptotic behavior as <<FORMULA_0256>>
 ...
 is greater than <<FORMULA_0259>>. The function $C$ of the query vector and the function $f$ depend on the problem. Interestingly, these bounds depend on $d$, the dimension spanned by the tokens, not the ambient dimension $n$.
 ...
 Notice that the one-layer transformer output is a linear combination of the uncorrupted samples. Hence, if the distribution $p_X$ is supported by a $d$-dimensional linear subspace, the estimate <<FORMULA_0261>> is also in that subspace. We can therefore look at convergence restricted to the supporting subspace. Therefore, it is the dimensionality of the supporting subspace that matters.

 Let a $d$-dimensional vector space $V$ be a linear subspace of $\R^n$. We define the maximum norm for $V$ with respect to some orthonormal basis <<FORMULA_0262>> in $V$ as <<FORMULA_0263>> for any <<FORMULA_0264>>. The conventional maximum norm for $\R^n$, of course, is defined  with respect to the standard orthonormal basis  <<FORMULA_0265>>. 
 Since <<FORMULA_0266>>, for all $i$,
 <<FORMULA_0267>>
 Then, for any <<FORMULA_0268>>,
 <<FORMULA_0269>>, since
 <<FORMULA_0270>>, for all <<FORMULA_0271>>. Thus, controlling component-wise error in any orthonormal basis in $V$ controls component-wise error in $\R^n$, in an $n$-independent but $d$-dependent manner. In the following, we give a flavor of how we can analyze finite sample estimate errors in $V$. The maximum norm <<FORMULA_0272>> is to be understood as $||\cdot||_{\infty,V}$ for some orthonormal basis choice. Here is the result relevant to the linear case described Subsubsection \ref{case1}.

 \begin{prop}
\label{prop:linear-case-rate}
Let <<FORMULA_0273>> and let <<FORMULA_0274>>
Then, for any <<FORMULA_0275>>
<<FORMULA_0276>>
for some <<FORMULA_0277>>.
\end{prop}
\begin{proof} We start by bounding the maximum norm of the difference,
<<FORMULA_0278>>
where $||\cdot||_{\text{op}}$ is the operator norm.

It can be shown that, for any <<FORMULA_0279>>
<<FORMULA_0280>>
for some <<FORMULA_0281>> \cite{rigollet2023high}. Combining with the first bound, we get our result.
\end{proof}
...
In the following, we assume that the support of $p_X$ is inside a vector space $V$ whose dimension we denote by $d$ (instead of <<FORMULA_0286>>, as in the sphere problem). In addition, we refer to the projection of the query on $V$ by <<FORMULA_0287>>, instead of <<FORMULA_0288>>. As usual, the maximum norm in $V$ is with respect to some orthonormal basis choice

\begin{prop}
\label{prop:nonlinear-case-rate}
Let <<FORMULA_0289>> and <<FORMULA_0290>> for <<FORMULA_0291>>. 

Then, for any <<FORMULA_0292>>
<<FORMULA_0293>>
and 
<<FORMULA_0294>>
\end{prop}
...
For the average <<FORMULA_0295>>, each term in the sum is bounded above and below by $e^{\pm\frac{R||\tilde x||_2}{\sigma_Z^2}}$.
So, the Hoeffding inequality leads to
<<FORMULA_0296>>
Setting <<FORMULA_0297>>, we get <<FORMULA_0298>>, which gives our first probabilistic inequality.

For each component of the vector average <<FORMULA_0299>>, the terms in the sum are bounded above and below by <<FORMULA_0300>>. We use similar arguments involving the Hoeffding inequality, combined with the union bound over all $d$ coordinates 
<<FORMULA_0301>> Once more, setting the RHS to $\delta$ and solving for $\epsilon$, we get our second probabilistic inequality.
\end{proof}
...
<<FORMULA_0302>>
where <<FORMULA_0303>> is a normalizing factor, with <<FORMULA_0304>>. The notation $\mathbbm{1}_L$ stands for an $L$-dimensional vector of ones.
...
Using Lemma \ref{appendix:small-arg}, as <<FORMULA_0308>>,
<<FORMULA_0309>>
where <<FORMULA_0310>> is the empirical mean and the notation $\mathbbm{1}_L$ emphasizes that it is a column vector of ones with dimension $L$. 
...
%\( \beta \) (or large $W_{KQ}$)}
As <<FORMULA_0311>>, the softmax function simply selects the maximum over its inputs (as long as the the maximum is unique):
<<FORMULA_0312>>
...
\caption{
  ...
  The MSE is numerically evaluated by assuming scaled identity weights <<FORMULA_0313>> (x-axis) and <<FORMULA_0314>> (y-axis) and scanning over a $50 \times 50$ grid. The green point corresponds to the heuristic minimizer identified from the posterior mean. In Case 2 it is exact, while in case 3 it is an approximation that neglects the residual term (see Proposition \ref{prop:bayes-case3}). The orange point corresponds to the learned weights displayed in Fig. \ref{fig:empirical-training-triple}(b), while the white point corresponds to the numerically identified minimum from this 2D scan. These can fluctuate due to the finite context (<<FORMULA_0315>>) and sampling (<<FORMULA_0316>> here). In both panels, it is apparent that the trained weights and the heuristic estimator co-occur in a broad valley (contour) of the loss landscape. 
  }
...
Focusing on the subspace denoising case, suppose each prompt is transformed by a fixed invertible square matrix $A$, i.e. <<FORMULA_0317>>.
...
<<FORMULA_0318>>
where <<FORMULA_0319>> as before. 

\begin{figure}[h!]
\centering
\includegraphics[width=0.92\textwidth]{figs/fig-appendix-non-identity-weights.pdf}
\caption{
    (a) Example transformation $A$ used to globally alter the in-context denoising prompts. (b) Structure of the optimal attention weights for this transformed subspace-denoising task. (c,d) Empirically, we find that both linear attention and softmax attention layers are able to learn these structured targets, but with distinct scalings <<FORMULA_0320>>. Final weights after 500 epochs using Adam, random initializations, and context length <<FORMULA_0321>>; other parameters follow Fig. \ref{fig:empirical-training-triple}(a).
  }
\label{fig:non-identity-weights}
\end{figure}

Notably, we find that both the linear and softmax attention layers are able to learn these structures; see Fig. \ref{fig:non-identity-weights} for an example. We use the same basic training procedure as the limiting case of <<FORMULA_0322>> (no additional coordinate transformation) assumed throughout the main text. 

Suppose we still work with transformed coordinates <<FORMULA_0323>> and <<FORMULA_0324>>, but now intend to retrieve <<FORMULA_0325>> in the new coordinate space (rather than $X_{L+1}$ as above). In this case, we would be dealing with variables with covariance matrices <<FORMULA_0326>>. We would need weight matrices that are not simply proportional to identity to deal with the covariance structure. This is also the case for in-context learning of linear functions when the input has an anisotropic covariance matrix \cite{bartlett2024jmlr, ahn2023transformers}. 
Recall in the original setting, we had the sample covariance
<<FORMULA_0327>> and noise <<FORMULA_0328>>, leading to the estimator, Eq. (\ref{eq:main-case1-sec3}): 
<<FORMULA_0329>> . 
Here, the sample covariance is <<FORMULA_0330>>, and the noise <<FORMULA_0331>> has covariance <<FORMULA_0332>>.
One can show the generalized solution is <<FORMULA_0333>>.
Thus, in the transformed coordinates, the denoising estimate is
<<FORMULA_0334>>

For the relationship of this denoising result in $Y$ to energy models, as discussed in Section~\ref{sec:assoc-mem} and Subsection~\ref{appendix:linear-attention-trad-Hopfield}, we need a modified energy <<FORMULA_0335>> and a preconditioner proportional to <<FORMULA_0336>>.
...
We have considered a trained network with linear attention, relating the query <<FORMULA_0337>> and the estimate of the target <<FORMULA_0338>>, of the form 
<<FORMULA_0339>>
with <<FORMULA_0340>>. 

With 
<<FORMULA_0341>>
gradient descent iteration <<FORMULA_0342>> gives us <<FORMULA_0343>> 
...
For random memory patterns $X_{1:L}$, and the query denoting Ising spins <<FORMULA_0344>>, the so-called Hopfield energy is 
<<FORMULA_0345>>
We could relax the Ising nature of the spins by letting <<FORMULA_0346>>, with a constraint <<FORMULA_0347>>. This is the spherical model \cite{fischer1993spin} since the spin vector $s$ lives on a sphere. If we minimize this energy the optimal $s$ would be aligned with the dominant eigenvector of the matrix  <<FORMULA_0348>> \cite{fischer1993spin}, and the model will not have a retrieval phase (see \citet{bolle2003spherical} for a similar model that does). A soft-constrained variant can also be found in Section 3.3, Model C of  \citet{krotov2021large}.

We could reformulate the optimization problem of minimizing the Hopfield energy, subject to <<FORMULA_0349>>, as
<<FORMULA_0350>>
The $s$-dependent part of the Lagrangian, with $\lambda$ replaced by <<FORMULA_0351>> gives us the energy function in Eq. \ref{eq:spherical-Hopfield-appendix} which we have called the Naive Spherical Hopfield model. 

<<FORMULA_0352>>


For $L$ much larger than $n$, <<FORMULA_0353>>, so its eigenvalues are either 0 or are very close to $\sigma_0^2$. Hence, for large $L$ and <<FORMULA_0354>>, this quadratic function is very likely to be positive definite.  One-step gradient descent brings $s$ down to the $d$-dimensional linear subspace $S$ spanned by the patterns, but repeated gradient descent steps would take $s$ towards zero.


\subsection{Remarks on the softmax attention case (mapping to dense associative memory networks)}

Regarding the mapping discussed in the main text, we note that there is a symmetry condition on the weights <<FORMULA_0355>> that is necessary for the softmax update to be interpreted as a gradient descent (i.e. a conservative flow). 
In general, a flow <<FORMULA_0356>> is conservative if it can be written as the gradient of a potential, i.e. 
<<FORMULA_0357>> for some $V$. For this to hold, the Jacobian of the dynamics <<FORMULA_0358>> must be symmetric. 

The softmax layer studied in the main text is <<FORMULA_0359>>.
We will denote <<FORMULA_0360>> and <<FORMULA_0361>>, both in $\mathbb{R}^L$. 
Then the Jacobian is
<<FORMULA_0362>>

Observe that <<FORMULA_0363>> is symmetric (keeping in mind that <<FORMULA_0364>> depends on $W_{KQ}$). The Jacobian symmetry requirement <<FORMULA_0365>> therefore places the following constraint on feasible <<FORMULA_0366>>:
<<FORMULA_0367>>
...
The mapping discussed in the main text involves discrete gradient descent steps, Eq. (\ref{eq:DAM-GD-update}). In general, this update rule retains a ``residual" term in <<FORMULA_0368>> if we choose a different descent step size <<FORMULA_0369>>. Thus, taking $K$ recurrent updates could be viewed as the depthwise propagation of query updates through a $K$-layer architecture if one were to use tied weights. Analogous residual streams are commonly utilized in more elaborate transformer architectures to help propagate information to downstream attention heads.

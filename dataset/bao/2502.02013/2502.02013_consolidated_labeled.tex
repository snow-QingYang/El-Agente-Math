...
%Let <<FORMULA_0001>> represent a batch of $N$ samples, each with dimensionality $D$. The vector $z_i$ denotes the $i$-th row of $Z$. We denote the $i$-th largest eigenvalue of a matrix $\mathbf{M}$ as <<FORMULA_0002>>, and the trace of $\mathbf{M}$ by <<FORMULA_0003>>. Input sequences are denoted by <<FORMULA_0004>> and output sequences by <<FORMULA_0005>>, where $L$ is the sequence length and $d$ is the feature dimension. 
...
Consider a neural network that maps inputs $\mathbf{x}$ (e.g., tokens in a sequence) to internal hidden states $\mathbf{Z}$. We denote <<FORMULA_0006>> as a matrix of $N$ data samples (or tokens) in $D$ dimensions. Some key questions arise: 
%We consider a batch of $N$ samples, each represented by a $D$-dimensional vector. Let <<FORMULA_0007>> be the matrix of representations, where $z_i$ denotes the $i$-th row of $\mathbf{Z}$. For a matrix $\mathbf{M}$, we use <<FORMULA_0008>> to denote its $i$-th largest eigenvalue, and <<FORMULA_0009>> to denote its trace. When dealing with sequences, we let <<FORMULA_0010>> represent the input sequence and <<FORMULA_0011>> the output sequence, where $L$ is the sequence length and $d$ is the feature dimension.
...
% \end{tcolorbox}
%Let <<FORMULA_0012>> be a matrix of \(N\) data samples (e.g., prompts or tokens), each embedded in a \(D\)-dimensional space. We define the \emph{Gram matrix} <<FORMULA_0013>>, whose eigenvalues <<FORMULA_0014>> reflect how variance is distributed across principal directions of \(\mathbf{Z}\). 
We focus on a key quantity known as \textit{matrix-based entropy} \citep{giraldo2014measures, skean2023dime}, which applies directly to the Gram matrix <<FORMULA_0015>>. Let <<FORMULA_0016>> be the (nonnegative) eigenvalues of $\mathbf{K}$. For any order <<FORMULA_0017>>, define:
<<FORMULA_0018>>
where <<FORMULA_0019>>. Intuitively, if only a few eigenvalues dominate, <<FORMULA_0020>> is \emph{small}—indicating a highly compressed representation. Conversely, if $\mathbf{Z}$ is spread out across many principal directions, <<FORMULA_0021>> is \emph{large}. By varying $\alpha$, one smoothly transitions between notions like collision entropy (<<FORMULA_0022>>) and von Neumann entropy (<<FORMULA_0023>>). We will typically use <<FORMULA_0024>> for simplicity. 
...
%We focus on a core quantity known as \textit{matrix-based entropy} \citep{giraldo2014measures, skean2023dime}, which applies directly to the Gram matrix <<FORMULA_0025>>. Let <<FORMULA_0026>> be the (nonnegative) eigenvalues of $\mathbf{K}$. For any order <<FORMULA_0027>>, define:

\iffalse
A key tool for analyzing these embeddings is the \emph{\(\alpha\)-order matrix-based entropy}~\citep{giraldo2014measures}. For <<FORMULA_0028>>,
<<FORMULA_0029>>
where <<FORMULA_0030>> <<FORMULA_0031>>.  Intuitively, if only a few eigenvalues dominate \(\mathbf{K}\), then \(S_{\alpha}\) is \emph{small}, indicating that \(\mathbf{Z}\) is ``compressed'' into a low-dimensional subspace. Conversely, if eigenvalues are spread more evenly, <<FORMULA_0032>> is \emph{large}, reflecting a richer or more diverse representation. 
As $\alpha$ changes, it smoothly interpolates among other well-known measures such as collision entropy or min-entropy. We show the behavior of \eqref{eq:matrix-based-entropy} for varying $\alpha$ in the Appendix. An important case is <<FORMULA_0033>>,  where \(S_{\alpha}\) converges to the so-called von Neumann entropy of \(\mathbf{K}\)  defined as below. Unless otherwise specified, we use <<FORMULA_0034>> (i.e., rank-based Shannon entropy) in subsequent results.
<<FORMULA_0035>>
\fi
...
% Many popular evaluation metrics can be shown to be special cases or close relatives of <<FORMULA_0036>>. For instance, the so-called Effective Rank~\cite{effective-rank} is closely tied to <<FORMULA_0037>>, highlighting how dimensionality effectively shrinks if the representation is strongly compressed. We prove this connection later in Theorem~\ref{thm:effective-rank-bound}. This has implications for representation evaluation metrics such as RankMe~\cite{garrido2023rankme} and LiDAR~\cite{thilak2023lidar} which are both inspired by Effective Rank.
%    \begin{itemize}
...
\begin{itemize}[itemsep=1pt, topsep=0pt]
    ...
    A handful of large eigenvalues in <<FORMULA_0039>> indicates that $\mathbf{Z}$ is low-rank, i.e.\ the model has collapsed much of the input variation into fewer dimensions. In contrast, a more uniform eigenvalue spectrum implies higher-entropy, more diverse features.
    ...
    Metrics like InfoNCE~\citep{oord2018representation} and LiDAR~\citep{thilak2023lidar} effectively measure whether augmentations of the same sample (e.g.\ character swaps) map to \emph{similar} embeddings. Strong invariance corresponds to stable clustering in <<FORMULA_0040>>, which again depends on the distribution of eigenvalues and how local vs.\ global features are retained or discarded.

\end{itemize}

Thus, evaluating <<FORMULA_0041>> provides a single lens for assessing “representation quality” across compression, geometric structure, and invariance—and highlights how \emph{both} local details and global patterns are organized.


%Thus, evaluating <<FORMULA_0042>> helps unify the notion of “representation quality” across different angles.
...
%We follow the work of~\citep{wei2024large} and apply Eq.~\eqref{eq:matrix-based-entropy} to the set of token embeddings \emph{within a single prompt} <<FORMULA_0043>>. We call the resulting <<FORMULA_0044>> the \emph{prompt entropy}, since it tracks how much the model spreads out (or compresses) the token embeddings in that prompt. Higher values indicate more distinct embeddings for each token, while lower values indicate greater redundancy or compression. 
...
%We can also aggregate all embeddings from a batch (or entire dataset) to measure global diversity. Similar to prompt entropy, we compute the matrix-based entropy on <<FORMULA_0045>> for $N$ prompts. To aggregate embeddings, we take the mean token (excluding padding) for every prompt and insert it as a row in <<FORMULA_0046>>. Dataset entropy reveals whether the model lumps many different inputs into similar embeddings (low dataset entropy) or better distinguishes them (high dataset entropy).

We can also aggregate embeddings \emph{across N prompts} by taking the mean token embedding of each prompt to form <<FORMULA_0047>>. Applying entropy to <<FORMULA_0048>> yields a \emph{dataset}-level measure of global diversity—revealing how distinctly the model separates different inputs.




\paragraph{Effective Rank.} ~\cite{effective-rank} can be shown to be a lower bound to <<FORMULA_0049>>, highlighting how dimensionality effectively shrinks if the representation is strongly compressed. We prove this connection in Theorem~\ref{thm:effective-rank-bound}. This has implications for popular representation evaluation metrics such as RankMe~\cite{garrido2023rankme} and LiDAR~\cite{thilak2023lidar}, which are both inspired by Effective Rank.
...
% \paragraph{Dataset Entropy.}
% We can also aggregate all embeddings from a batch (or entire dataset) to measure global diversity. Similar to prompt entropy, we compute the matrix-based entropy on <<FORMULA_0051>> for $N$ prompts/tokens. This reveals whether the model lumps many different inputs into similar embeddings (low dataset entropy) or better distinguishes them (high dataset entropy).

\subsubsection{Geometric Metrics}
\paragraph{Curvature.}

Proposed by \citet{hosseini2024curvature}, \emph{curvature} captures how sharply the token embeddings turn when viewed as a sequence in $\mathbb{R}^D$. For a prompt of length $L$, let <<FORMULA_0052>> be the difference between consecutive tokens. The average curvature is:
<<FORMULA_0053>>
...
\iffalse
Proposed by \citet{hosseini2024curvature}, curvature measures how the direction between consecutive token embeddings changes. For $L$ tokens <<FORMULA_0054>>, define <<FORMULA_0055>>. The curvature over the prompt is
<<FORMULA_0056>>
...
%The InfoNCE loss~\citep{oord2018representation} is defined such that lower values correspond to higher mutual information between paired augmentations <<FORMULA_0060>> relative to negative pairs. A \emph{lower} InfoNCE loss indicates stronger invariance to augmentation.
...
% For <<FORMULA_0061>>, <<FORMULA_0062>> in Eq.~\eqref{eq:matrix-based-entropy} is Schur-concave with respect to the ordered eigenvalues of <<FORMULA_0063>>. 
% \end{theorem}
...
For Shannon-based entropy (<<FORMULA_0064>>),
<<FORMULA_0065>>
...
\begin{theorem}[Lower Bound via Effective Rank]
\label{thm:effective-rank-bound}
Let <<FORMULA_0066>> and <<FORMULA_0067>> have singular values <<FORMULA_0068>>. Denote Shannon-based matrix entropy by <<FORMULA_0069>>. Then
<<FORMULA_0070>>
meaning high <<FORMULA_0071>> entropy implies a large effective rank. 
\end{theorem}
\fi
...
% \begin{itemize}
%     \item \textbf{Prompt Entropy:} Measured per prompt, using a token-level embedding matrix <<FORMULA_0072>> (where $L$ is the prompt length). 
%     \item \textbf{Dataset Entropy:} Measured across multiple prompts, by aggregating each prompt’s representation into a single vector, forming <<FORMULA_0073>> (where $N$ is the total number of prompts).
% \end{itemize}
...
\mbox{}
\begin{enumerate}[itemsep=1pt, topsep=0pt]
\item If \emph{prompt entropy} remains near its maximum for \emph{all} prompts, then the \emph{dataset entropy} <<FORMULA_0074>> grows on the order of 
<<FORMULA_0075>>
...
\label{thm:nce-bound}
For data <<FORMULA_0077>> and representation <<FORMULA_0078>>, the InfoNCE loss \citep{oord2018representation} on \(N\) samples satisfies:
<<FORMULA_0079>>
where <<FORMULA_0080>> can be interpreted as a (dataset-level) matrix-based entropy. Hence, lowering InfoNCE is consistent with learning a representation \(Z\) of higher overall entropy, underscoring the alignment between invariance metrics and the geometry of \(\mathbf{Z}\).
\end{theorem}
\fi
\begin{theorem}[Dataset Entropy Bounds InfoNCE]\label{thm:nce-bound}
For data $X$ and representation <<FORMULA_0081>>, the InfoNCE loss on $N$ samples satisfies:
<<FORMULA_0082>>
where <<FORMULA_0083>> is interpretable as matrix-based entropy at the dataset level. Hence, reducing InfoNCE implies learning a higher-entropy (and thus often more robust) representation.
\end{theorem}
...
Overall, our theoretical analysis shows that \emph{compression} (entropy), \emph{geometry} (curvature, rank), and \emph{invariance} (e.g.\ InfoNCE) are all facets of how the Gram matrix <<FORMULA_0084>> distributes variance. Examining these metrics across different layers reveals exactly where a network “prunes” redundancy (low entropy) versus preserving essential distinctions (high entropy). This unified perspective also facilitates cross-architecture comparisons (e.g.\ transformers vs.\ SSMs) by highlighting how each architecture organizes information internally. Beyond offering a theoretical foundation, it provides a practical blueprint for diagnosing, tuning, and improving hidden-layer representations. 
...
\iffalse
\begin{enumerate}
\item \textbf{Single unifying view.} Many metrics in language and vision—RankMe, InfoNCE, curvature, collision entropy—derive from how <<FORMULA_0085>> behaves. Viewing them as reflections of Eq.~\eqref{eq:matrix-based-entropy} helps identify when multiple metrics will agree or diverge.
...
Consider a batch of $N$ input sequences (or tokens) that map via a neural network to an internal representation matrix <<FORMULA_0086>>. We think of each row of $\mathbf{Z}$ as an embedding of one input sequence (or token). A key quantity is the (linear) Gram matrix <<FORMULA_0087>>, whose eigenvalues characterize the variance distribution across principal directions in the embedding space.
\iffalse
...
<<FORMULA_0088>>
where <<FORMULA_0089>> denotes the $i$-th eigenvalue of $\mathbf{K_Z}$ and <<FORMULA_0090>>. As <<FORMULA_0091>>, <<FORMULA_0092>> recovers Shannon entropy (specifically von Neumann entropy in matrix form). As $\alpha$ changes, it smoothly interpolates among other well-known measures such as collision entropy or effective rank \citep{garrido2023rankme}. 

When $\mathbf{Z}$ is highly compressed (i.e., embeddings are concentrated in a small subspace), a few eigenvalues dominate, lowering <<FORMULA_0093>>. Conversely, a more ``spread out'' representation has a more uniform eigenvalue distribution and thus higher entropy.


\textbf{Bridging geometry and invariance.}
Importantly, many widely used metrics can be shown to be specializations of <<FORMULA_0094>> or to be closely related to how $\mathbf{Z}$ changes under data augmentations. This leads to three practical categories of measures we will use throughout:
...
We apply Eq.~\eqref{eq:matrix-based-entropy} to the set of token embeddings \emph{within a single prompt} <<FORMULA_0095>>, forming <<FORMULA_0096>>. We call the resulting <<FORMULA_0097>> the \emph{prompt entropy}, since it tracks how much the model spreads out (or compresses) the token embeddings in that prompt. Higher values indicate more distinct embeddings for each token, while lower values indicate greater redundancy or compression. 

Unless otherwise specified, we use <<FORMULA_0098>> (i.e., rank-based Shannon entropy). Alternative $\alpha$ values can emphasize tail eigenvalues more or less (Appendix provides details).
...
\paragraph{Dataset Entropy.}
We can also aggregate all embeddings from a batch (or entire dataset) to measure global diversity. Similar to prompt entropy, we compute the matrix-based entropy on <<FORMULA_0099>> for $N$ prompts/tokens. This reveals whether the model lumps many different inputs into similar embeddings (low dataset entropy) or better distinguishes them (high dataset entropy).

\paragraph{Curvature.}
Proposed by \citet{hosseini2024curvature}, curvature measures how the direction between consecutive token embeddings changes. For $L$ tokens <<FORMULA_0100>>, define <<FORMULA_0101>>. The curvature over the prompt is
<<FORMULA_0102>>
High curvature suggests that consecutive embeddings shift direction abruptly, indicating less ``smoothness'' in the layer’s representation. 

\subsubsection{Augmentation Invariance Metrics}
\label{sect:invariance-metrics}
We also evaluate how robust a representation is to perturbations in the input. Suppose we have a set of $N$ prompts <<FORMULA_0103>>. For each $p_i$, we create two perturbed versions $p_i^{(a)}$ and $p_i^{(b)}$ using standard text augmentation (e.g., random character swaps, keyboard-level changes; see Appendix). Let <<FORMULA_0104>> be the resulting embeddings for each augmented set, where the $i$-th row in each corresponds to the same original prompt $p_i$.

We then measure \emph{how similar} these two embedding sets are under different scoring criteria:

\paragraph{InfoNCE.}
The InfoNCE loss~\citep{oord2018representation} is defined such that lower values correspond to higher mutual information between paired augmentations <<FORMULA_0105>> relative to negative pairs. A lower InfoNCE loss thus indicates \emph{stronger} invariance to perturbations.
...
%We define <<FORMULA_0106>> and <<FORMULA_0107>> are the input and output sequences, respectively.
...
Following \citet{wei2024large}, we use the $\alpha$-order matrix-based entropy \citep{giraldo2014measures} as a surrogate for Rényi entropy. For a sequence of token representations <<FORMULA_0108>>, the Gram matrix is <<FORMULA_0109>>. The entropy is:
...
Unless otherwise specified, we use the limit case <<FORMULA_0111>> in our calculations. At this limit, the metric is equivalent to the RankMe measure defined in \citet{garrido2023rankme}. We explore the effects of different $\alpha$ values in Appendix \ref{appendix:entropy}. For a more in-depth examination of prompt entropy, refer to Appendix~\ref{sect:appendix-prompt-entropy}.
...
As introduced by \citet{hosseini2024curvature}, curvature measures how rapidly the direction between two adjacent token embedding vectors changes. Define their difference as <<FORMULA_0112>>. The average curvature of a prompt is:
...
These metrics measure how consistently a model represents a prompt when it is perturbed or augmented. Because augmentations may change prompt length, we average all token embeddings to form a single vector per prompt.
 ...
 Let <<FORMULA_0114>> and <<FORMULA_0115>> represent two augmented sets of $N$ prompts, where the $i$-th row in both corresponds to the same original prompt. Details on the augmentation process are in Appendix~\ref{appendix:prompt-augmentation}.


% We refer to the two batches of augmented prompts as <<FORMULA_0116>> and <<FORMULA_0117>>, where $N$ is the batch size and row $i$ in both matrices correspond to the same original prompt. We provide full details and examples of the augmentation process in Appendix~\ref{appendix:prompt-augmentation}.
...
To compute the LDA matrix, LiDAR uses augmentations to construct the class scatter matrix. In our setup, we use $N$ classes (one for each prompt) and <<FORMULA_0118>> samples per class. This is a larger sample size than the <<FORMULA_0119>> used in DiME or InfoNCE, reflecting the more complex requirements of computing the LDA matrix.

\paragraph{DiME}
DiME~\citep{skean2023dime} compares the alignment of paired samples to that of randomly paired samples. Similar to InfoNCE, it is used to estimate the mutual information between two augmented sets of prompts. DiME is grounded in the matrix-based entropy defined in Eq.~\ref{eq:matrix-based-entropy}. In essence, it quantifies how closely the pairings in <<FORMULA_0120>> resemble each other, compared to pairings of <<FORMULA_0121>> for a permutation matrix $\Pi$. Higher DiME values imply that correct augmentation pairs yield representations that are significantly more similar than random pairings, indicating stronger augmentation invariance.
\fi
\subsection{Theory}
\fi
...
where <<FORMULA_0124>> and <<FORMULA_0125>> are learned weights.
...
where <<FORMULA_0129>>, <<FORMULA_0130>>, <<FORMULA_0131>>, and <<FORMULA_0132>> are learned parameters.
...
\begin{figure}[!b]
  ...
  \end{center}
  \caption{The behavior of Eq. \ref{eq:matrix-based-entropy} for varying values of $\alpha$ on Gram matrices with eigenvalues distributed with a $\beta$-power law such that <<FORMULA_0133>>.}
...
We follow the work of \cite{wei2024large} and use $\alpha$-order matrix-based entropy \cite{giraldo2014measures, skean2023dime, skean2024frossl}, which serves as a tractable surrogate for traditional Rényi’s $\alpha$-order entropy \cite{renyi1961measures}. The quantity is calculated using a similarity kernel $\kappa$ on a batch of samples drawn from a distribution, without making explicit assumptions on what the true distribution is. The choice of kernel $\kappa$ is flexible and can be any infinitely divisible kernel such as the Gaussian kernel, linear kernel, or Laplacian kernel, among others. For this work, we restrict ourselves to the linear kernel <<FORMULA_0134>>. This choice is motivated by the linear representation hypothesis \cite{parklinear2024} which finds that large language model representations encode high-level concepts such as truth \cite{burns2022dl}, honesty \cite{mallen2024eliciting}, and part-of-speech \cite{mamou2020emergence} in linearly separable manifolds.

 The equation for matrix-based entropy was previously defined in Eq. \ref{eq:matrix-based-entropy}. One way to interpret Eq. \ref{eq:matrix-based-entropy} is as the $\alpha$-order Rényi entropy of the Gram matrix eigenvalues\footnote{The non-zero eigenvalues of the Gram matrix <<FORMULA_0135>> are equivalent to those of the covariance matrix <<FORMULA_0136>>. Using the covariance matrix instead of the Gram matrix in Eq. \ref{eq:matrix-based-entropy} makes no difference and is more computationally efficient if <<FORMULA_0137>>.}. Notice how each eigenvalue is divided by <<FORMULA_0138>> before being raised to the $\alpha$ power. This is so that the eigenvalues of $\mathbf{K}_{\mathbf{Z}}$ sum to one (because  <<FORMULA_0139>>), which is a necessary condition to treat the eigenvalues as a probability distribution. Futhermore, each eigenvalue of $\mathbf{K}_{\mathbf{Z}}$ signifies the variance of samples in a particular principal component direction~\cite{scholkopf2018learning}. If entropy is low, then the eigenvalues form a heavy-tail distribution which implies that a few components dominate the variance of samples in $Z$. On the other hand, at maximum entropy, the eigenvalues form a uniform distribution and samples are spread equally in all directions. Matrix-based entropy is reminiscent of the LogDet entropy which uses the determinant of $\mathbf{K}_{\mathbf{Z}}$ to capture how much "volume" a dataset occupies~\cite{shwartz2023information, zhouyin2021understanding}. The LogDet entropy is given by <<FORMULA_0140>>. One can use Jensen's inequality to show that the LogDet entropy is a lower bound of Eq \ref{eq:matrix-based-entropy} when <<FORMULA_0141>> (Appendix J.4 of~\cite{shwartz2023information}).
 
 Depending on the choice of $\alpha$, several special cases of matrix-based entropy can be recovered. In particular, when <<FORMULA_0142>> it equals Shannon entropy (also referred to as von Neumann entropy in quantum information theory \cite{bach2022information, boes2019neumann}), and when <<FORMULA_0143>> it equals collision entropy. Interestingly, the case of <<FORMULA_0144>> can be calculated without explicit eigendecomposition \cite{skean2024frossl}. We show in the Appendix Figure \ref{fig:power_law_entropy} how varying values of $\alpha$ affect the matrix-based entropy of Gram matrices with eigenvalues distributed with a $\beta$-power law such that <<FORMULA_0145>>. It is shown that for larger values of $\alpha$, smaller eigenvalues contribute more to the entropy.
 

%The self-attention block of the transformer ~\citep{vaswani2017attention} consists of key (<<FORMULA_0146>>), query(<<FORMULA_0147>>) and value (<<FORMULA_0148>>) that are learned by transforming input $x$ using the following weight matrices <<FORMULA_0149>>, <<FORMULA_0150>> and <<FORMULA_0151>>. They are computed as follows:
%<<FORMULA_0152>>
...
% where <<FORMULA_0156>>, <<FORMULA_0157>>, <<FORMULA_0158>>, and <<FORMULA_0159>> are learned parameters.
...
%where $h_i$ denotes the latent state of the system, and the dynamic matrices <<FORMULA_0161>> of corresponding dimensions represent the parameters learned by the model.
...
%  Depending on the choice of $\alpha$, several special cases of matrix-based entropy can be recovered. In particular, when <<FORMULA_0162>> it equals Shannon entropy (also referred to as von Neumann entropy in quantum information theory \cite{bach2022information, boes2019neumann}), and when <<FORMULA_0163>> it equals collision entropy. Interestingly, the case of <<FORMULA_0164>> can be calculated without explicit eigendecomposition \cite{skean2024frossl}. We show in the Appendix Figure \ref{fig:power_law_entropy} how varying values of $\alpha$ affects the matrix-based entropy of Gram matrices with eigenvalues distributed with a $\beta$-power law such that <<FORMULA_0165>>. It is shown that for larger values of $\alpha$, smaller eigenvalues contribute more to the entropy.
...
\begin{itemize}
    \item (<<FORMULA_0166>>) \hspace{3pt} Mint records indicate the first gold dollars were produced on May 7...
    \item (<<FORMULA_0167>>) Mint records indicate the first gold dollars were Mint Mint May 7...
    \item (<<FORMULA_0168>>) Mint records Mint Mint Mint gold dollars were Mint Mint Mint 7...
    \item (<<FORMULA_0169>>) Mint Mint Mint Mint Mint Mint Mint Mint Mint Mint Mint Mint Mint...
\end{itemize}
...
\begin{itemize}
    \item (<<FORMULA_0170>>) \hspace{3pt} Mint records indicate the first gold dollars were produced on May 7...
    \item (<<FORMULA_0171>>) Mint records indicate salivary first gold dollars were produced on May NaCl...
    \item (<<FORMULA_0172>>) Mint records Dallas actively first dollars persufors on Mayder129 18...
    \item (<<FORMULA_0173>>) arf emulsion minorensteinorianmega\_TOStack potsRecip Installifykeeping...
\end{itemize}

% \subsection{Random Prompts with Certain Length}

% To make a random prompt of a specific length $T$, we sample $T$ tokens uniformly from the Pythia tokenizer distribution. Such a prompt may look like the following for <<FORMULA_0174>>: "Proposition Sequencespecific Exp fibers brows Club overviewNos toss Thinking traderMulti indoorlis".
...
% To create the LLM fractal, a list is constructed of negative log-likelihoods denoted as <<FORMULA_0175>> where each list element is defined as:
% <<FORMULA_0176>>
...
\begin{definition}{(Majorization)} Let <<FORMULA_0178>> be nonnegative vectors such that <<FORMULA_0179>>. We say that q majorizes p, denoted by <<FORMULA_0180>>, if their ordered sequences <<FORMULA_0181>> and <<FORMULA_0182>> satisfy:

<<FORMULA_0183>>
\end{definition}

\begin{definition}{(Schur-Convexity)} A real-valued function $f$ on $\mathbb{R}^n$ is called Schur-convex if <<FORMULA_0184>>, and Schur-concave if <<FORMULA_0185>>.
\end{definition}

\begin{lemma} 
The matrix-based entropy, as given in Equation~\ref{eq:matrix-based-entropy}, is a Schur-concave function for <<FORMULA_0186>>. This result is well-known and, for instance, was recently given by Lemma 4.1 in \citep{giraldo2014measures}.
\end{lemma}

\begin{theorem}
Suppose we have a matrix of embeddings <<FORMULA_0187>> and its covariance <<FORMULA_0188>>. Then the effective rank of $Z$ is an lower bound of <<FORMULA_0189>>, where $S_1$ denotes the matrix-based entropy of <<FORMULA_0190>>.

\end{theorem}
\begin{proof}
    Denote the ordered singular values of $Z$ as <<FORMULA_0191>> and the ordered eigenvalues of <<FORMULA_0192>> as <<FORMULA_0193>>. Without loss of generality, assume that <<FORMULA_0194>>. If this is not the case, then set <<FORMULA_0195>> and <<FORMULA_0196>>.
    
    It is straightforward to show that <<FORMULA_0197>>. Because <<FORMULA_0198>>, we have that <<FORMULA_0199>>. 
    This implies that <<FORMULA_0200>>. Therefore, <<FORMULA_0201>>.

\end{proof}

\begin{proposition}\textbf{(Random Unit Vectors are Nearly Orthogonal)}
Suppose we have $m$ unit vectors in $\R^D$, that are distributed according to the uniform distribution on the hyper-sphere. Then with probability at least <<FORMULA_0202>>, we have that for any pair <<FORMULA_0203>>, <<FORMULA_0204>>,
<<FORMULA_0205>>
\end{proposition}
\begin{proof}
    ...
    The probability of a uniformly distributed vector on the unit sphere not landing in <<FORMULA_0207>> can be bounded as,
	<<FORMULA_0208>>
    Now, treating $\mathbf{v_i}$ as $e_1$, the basis vector, without loss of generality, we have that, when <<FORMULA_0209>> are uniformly distributed on the hyper-sphere,
    <<FORMULA_0210>>
    Now, by the union bound on each <<FORMULA_0211>>, we get that,
	<<FORMULA_0212>>
    So then with probability at least <<FORMULA_0213>>, we have that, for any pair <<FORMULA_0214>>,
	<<FORMULA_0215>>
\end{proof}

\begin{theorem}
	(\textbf{Maximum Prompt Entropy implies Large Dataset Entropy.)}
	Suppose we have a orthogonally equivarient representation model $Z$ such that for all sequences <<FORMULA_0216>> the prompt entropy is maximal and the rows are unit. Suppose also that the data distribution $\mathbf{Data}$ is a isotropic unit Gaussian. Suppose we draw sequences of length <<FORMULA_0217>> from the data distribution. Then with probability <<FORMULA_0218>> over draw of <<FORMULA_0219>>, we have that,
	<<FORMULA_0220>>
\end{theorem}
\begin{proof}
	First note that, since the prompt entropy is maximal for each sample $ i $, which we denote <<FORMULA_0221>>, then the matrix <<FORMULA_0222>> is full rank. Since by assumption each row of $Z_i$ has unit rows, then we know that <<FORMULA_0223>>. In particular we also know that <<FORMULA_0224>> for all pairs <<FORMULA_0225>> by the assumption that the prompt entropy is maximized. In particular we then know that <<FORMULA_0226>> is a orthogonal matrix, and the rows of $Z_i$ form an orthonormal set. We can then write, for some $O_i$ a rotation matrix, that,
	<<FORMULA_0227>>
	We will denote the average over sequences of length $L$, across all $N$ samples, by the dataset matrix <<FORMULA_0228>>. Since by assumption our model <<FORMULA_0229>> is orthogonally equivariant, and the $\textbf{Data}$ distribution is radially symmetric, it follows that these <<FORMULA_0230>> are random points on the hypersphere of radius <<FORMULA_0231>>. This means that the matrix <<FORMULA_0232>> consists of rows that are uniform points on hypersphere of radius $1$. Now notice that,
	<<FORMULA_0233>>
	Since <<FORMULA_0234>> is a unit vector this will simplify to,
	<<FORMULA_0235>>
	Now notice that by proposition, we have that with probability at least <<FORMULA_0236>>,
	<<FORMULA_0237>>
	The union bound then tells us that,
	<<FORMULA_0238>>
	So then with probability at least <<FORMULA_0239>> over the draw of the data points, we have that,
	<<FORMULA_0240>>
	So then since,
	<<FORMULA_0241>>
	we have that, <<FORMULA_0242>>. In particular,
	<<FORMULA_0243>>
...
\textbf{(Minimal Prompt Entropy Implies Small Dataset Entropy.)}
	Suppose we have a orthogonally equivariant representation model $Z$ such that for all sequences <<FORMULA_0244>> the prompt entropy is minimal and the rows are unit. Suppose also that the data distribution $\mathbf{Data}$ is a isotropic unit Gaussian. Suppose we draw sequences from the data distribution. Then with probability <<FORMULA_0245>> over the draw of <<FORMULA_0246>>, we have that,
	<<FORMULA_0247>>
\end{theorem}

\begin{proof}
	Since the prompt entropy is minimal for each sample, we know that each <<FORMULA_0248>> will be a rank one matrix, so we can write it as the outer product. In particular, we can write <<FORMULA_0249>>. However, since the rows of <<FORMULA_0250>> are of unit length, we know that all the rows are identical, so we may write without loss of generality, <<FORMULA_0251>>. Then, it follows that, 
	<<FORMULA_0252>>
	We will write the dataset average matrix as before as <<FORMULA_0253>>. In particular the matrix <<FORMULA_0254>> has rows that are all unit vectors, and these are randomly distributed uniformly on the hyper-sphere. Now notice that,
	<<FORMULA_0255>>
	Now by the prior proposition, with probability at least <<FORMULA_0256>>, we know that, for all <<FORMULA_0257>>, 
	<<FORMULA_0258>>
...
\begin{theorem} \textbf{(Dataset Entropy Bounds InfoNCE)}
	Let <<FORMULA_0261>> be a discrete random variable distributed according to the data distribution. Let <<FORMULA_0262>> be the Markovian relation between $X$ and the representation $Z$. Then, the InfoNCE loss on $N$ samples from $\textbf{Data}$ satisfies,
	<<FORMULA_0263>> 
	The entropy <<FORMULA_0264>> is analogous to the Dataset Entropy. 
\end{theorem}

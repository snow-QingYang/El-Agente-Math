...
Notably, SB generalizes standard diffusion models transporting data between arbitrary distributions <<FORMULA_0002>> with fully nonlinear stochastic processes, seeking the unique path measure that minimizes the kinetic energy.
...
\end{figure}
For training these matching frameworks, we start from two datasets that lack pre-aligned pairs---that is, we sample only from the marginal <<FORMULA_0003>>---and the framework is trained to determine the optimal coupling in the sense of least-energy transportation.
This process represents an unsupervised learning approach, as no prior information about the coupling is provided.
However, there are other applications---such as image restoration \citep{liu20232} and protein docking \citep{somnath2023aligned}---in which we have prior knowledge of the coupling $\pi_{0,1}$, between pairs <<FORMULA_0004>> drawn presumably from the SB solutions. Consequently, using an unsupervised approach for such problems is not ideal, as it discards valuable information about the relationships between the samples of the two boundary distributions. An alternative approach suggests reframing these tasks as inverse problems. In this vein, \cite{liu20232} proposed recently <<FORMULA_0005>>, building diffusion bridge matching frameworks between coupled data to perform image restoration.
...
<<FORMULA_0006>>
where $\sQ$ is a Markovian reference measure. Hence the solution of the dynamic SB $\sP^\star$ is considered to be the closest path measure to $\sQ$. A very convenient, and heavily used property of $\sP^\star$ is its decomposition as <<FORMULA_0007>>, where $\pi^\star$ is the solution of the static SB <<FORMULA_0008>>, and $Q_{|0,1}$ is the Bridge of the reference measure for pinned points <<FORMULA_0009>> \citep{leonard2013survey}.
...
As discussed above, SB Matching algorithms \citep{shi2023diffusion, liu2024gsbm} have emerged recently, following advancements in matching frameworks \citep{liu2022let, peluchetti2023diffusion, lipman2022flow, liu2022flow}, rendering the solution of the SB problem significantly more tractable. Specifically, this family of algorithms solves \eq{van_DSB} by separating the training process into two alternating steps. The first step entails relaxing the boundary distributional constraints in \eq{van_DSB} into just two endpoints, by fixing the coupling $\pi_{0,1}$, and drawing pairs of samples <<FORMULA_0012>>.
<<FORMULA_0013>>

This optimization problem returns the optimized intermediate bridges between the drawn pairs. Subsequently, the parameterized drift <<FORMULA_0014>> is matched given the prescribed marginal path from the previous step, by minimizing the following variational gap 
<<FORMULA_0015>>
where <<FORMULA_0016>> is the drift associated with the fixed intermediate path. Propagation of the SDE according to the matched parameterized drift <<FORMULA_0017>> 
<<FORMULA_0018>>
induces the parameterized coupling $\pi_{0,1}^\theta$.
In the next iteration of the BM algorithm, this updated coupling is used to generate improved pairs of <<FORMULA_0019>> employed to refine the marginal path.  
...
We denote with <<FORMULA_0020>> the set with the aligned pairs, the Key-Point (KP) set, with $N$ being the number of pairs in the set. Usually, <<FORMULA_0021>>, where $M$ is the total number of samples. 
...
We start by sampling a pair of non-coupled samples <<FORMULA_0022>>.
Let us assume that the <<FORMULA_0023>> sample from the KP set <<FORMULA_0024>> is closest to $X_0$ in the source distribution. Then, the distance <<FORMULA_0025>> is computed and we proceed to compute the distance <<FORMULA_0026>>. 
Therefore, for a fixed cou pair <<FORMULA_0027>>, we define the Guidance function for the non-aligned pair <<FORMULA_0028>>, and  as 
<<FORMULA_0029>>
...
\subsection{Feedback Schrödinger Bridge Matching}
 We propose \textbf{Feedback Schrodinger Brige Matching} (FBSM) a novel matching framework, which leverages the information of partially, optimally paired datasets to guide the matching of the non-aligned samples. Our analysis starts from recasting \eq{static_SSOT} in a dynamic formulation in Sec. \ref{sec:dyn.obj.}. Subsequently, we adopt recent advances in dynamic SB frameworks, that propose a decomposition of the dynamic optimization problem into two components \citep{liu2024gsbm, shi2023diffusion}. Therefore, we separate the training phase into two stages: 1) the optimization of the intermediate path, conditioning on the endpoints <<FORMULA_0031>>, and 2) the optimization of the parameterized drift, which results in refining the coupling.
...
\begin{theorem}\label{th:dyn.obj.}
     Assume <<FORMULA_0032>>, with <<FORMULA_0033>>, and <<FORMULA_0034>>, and consider a stochastic random variable $X_t$ connecting $X_0$ and $X_1$, whose law is the continuous marginal probability path $p_t$ joining <<FORMULA_0035>>. 
     Additionally consider the KP pairs <<FORMULA_0036>> with their fixed interpolating paths.
...
\end{theorem}
Notice that our dynamic formulation is a modified and generalized version of \eq{van_DSB} to capture the guidance from the KP samples. For degenerate guidance term <<FORMULA_0038>>, one readily obtains the standard entropic analogue of the fluid dynamic formulation \citep{benamou2000computational,gentil2017analogy} in \eq{van_DSB} which also corresponds to the objective of SB objective \citep{de2021diffusion, shi2023diffusion}.
% \vspace{-.3cm}


\subsubsection{Intermediate Path Optimization}
As discussed above, the initial step of our methodology entails the optimization of the intermediate path. Let the marginal be expressed as a mixture of conditional probability paths <<FORMULA_0039>>. More explicitly, we sample non-coupled pairs <<FORMULA_0040>> and fix the coupling at the boundaries $\pi_{0,1}$. This results in relaxing the distributional constraints in \eq{lagrangian} to just two endpoints. 
...
Let the continuous marginal path $p_t$ satisfy the following decomposition <<FORMULA_0041>>. For optimizing with respect to the intermediate path, the coupling $\pi_{0,1}$ is frozen which results in \eq{lagrangian} being recasted as 
    <<FORMULA_0042>>
\end{proposition}
...
Hence, the term <<FORMULA_0044>> is interpreted as state-feedback that steers the non-coupled samples, by projecting the conditioned drift on the gradient of $G_{t|0}$
...
\begin{figure}
    ...
    \caption{Gradient Field of <<FORMULA_0045>> in the S-tunnel}
    \label{fig:grad_f}
\end{figure}
...
We solve \eq{cond.lagrang.} for each pair <<FORMULA_0046>> and later marginalize to construct $p_t$.
The conditioned probability path is approximated in a simulation-free manner as a Gaussian path <<FORMULA_0047>>, where <<FORMULA_0048>> is the interpolant function between the pinned endpoints <<FORMULA_0049>>, <<FORMULA_0050>>, and $\sigma$ is the standard deviation with <<FORMULA_0051>>.
...
<<FORMULA_0052>>
where $\nu$ is by definition of <<FORMULA_0053>>. 
...
Subsequently, we will match the parameterized drift <<FORMULA_0054>>, given the prescribed path $p_{t|0,1}$ of the uncoupled samples from the previous step, which will further improve the intermediate path in the next iteration.
...
The convex conjugate of the Lagrangian in \eq{cond.lagrang.} is defined as the Hamiltonian <<FORMULA_0055>>. 
The optimization with respect to the drift yields <<FORMULA_0056>>, where <<FORMULA_0057>>, and <<FORMULA_0058>> is given by
<<FORMULA_0059>>
...
\begin{proposition}\label{prop:breg.}
    Consider the optimized conditional drift <<FORMULA_0061>> following the optimization in \eq{cond.lagrang.}. The 
    parameterized drift <<FORMULA_0062>> is matched to <<FORMULA_0063>> given the prescribed marginal path by minimizing the variational gap expressed through the Bregman divergence: <<FORMULA_0064>>
    <<FORMULA_0065>>
    From Proposition \ref{prop:hamiltonian},  the parameterized drift can be expressed as  <<FORMULA_0066>>.
...
Our algorithm finds a solution <<FORMULA_0068>> to \eq{lagrangian} by iteratively optimizing \eq{cond.lagrang.}, and \eq{BM}. 
First, we draw pairs <<FORMULA_0069>>, and fix the parameterized coupling $\pi_{0,1}^\theta$.
Subsequently, to compute the guidance function, let the <<FORMULA_0070>> data point from the KPs set <<FORMULA_0071>> be closest to $x_0$ in the source distribution, the distance <<FORMULA_0072>> between the two samples is evaluated. Next, for every sampled time-step ${t_k}$, we compute the distance <<FORMULA_0073>> between $X_{t_k}$, and $X_{t_k}^{i}$, and evaluate <<FORMULA_0074>>.
Then, optimizing \eq{cond.lagrang.}, between the endpoints <<FORMULA_0075>> from the first step, yields the optimal conditional path <<FORMULA_0076>>, whose corresponding drift <<FORMULA_0077>> is obtained from \eq{cond.drift}. This drift ensures that the uncoupled data follow the trajectory of the KPs.
Lastly, we fix the marginal path and optimize the coupling, by matching the parameterized drift <<FORMULA_0078>> to the optimized 
drift of the uncoupled samples <<FORMULA_0079>>, using \eq{BM}. 
% using \eq{BM}, given the prescribed marginal $p_t$.
The optimized parameterized drift <<FORMULA_0080>> induces an improved coupling $\pi^\theta_{0,1}$ by propagating the dynamics through the SDE in  \eq{SDE_prel}.


\begin{algorithm}[t]
    \small\caption{\small Feedback Schr\"{o}dinger Bridge Matching (\textbf{FSBM})}\label{alg:FSBM}
    \begin{algorithmic}[1]
    \STATE {\bfseries Input}: Boundary distributions $\pi_0$, and $\pi_1$ and the pairs from the key-point (KP) set <<FORMULA_0081>>, and their fixed trajectories 
    \STATE Initialize <<FORMULA_0082>>, and $\pi_{0,1}$ as independent coupling <<FORMULA_0083>>.
    \STATE {\bfseries repeat}
        \STATE \hskip1em  Sample <<FORMULA_0084>> by forward propagating using <<FORMULA_0085>>, on time steps <<FORMULA_0086>>
        \STATE \hskip1em  Find closest aligned sample $x_0^{i}$ to $x_0$, and calculate <<FORMULA_0087>>
        \STATE \hskip1em  Using the same <<FORMULA_0088>> data point from the KP set, calculate <<FORMULA_0089>>
        \STATE \hskip1em  <<FORMULA_0090>> from \eq{G-fun}
        \STATE \hskip1em  Fix the <<FORMULA_0091>>, and obtain <<FORMULA_0092>> using \eq{cond.lagrang.}
        \STATE \hskip1em  Calculate $u_{t|0,1}$ from \eq{cond.drift}
        \STATE \hskip1em  Update <<FORMULA_0093>>, from \eq{BM}
...
\vspace{-1.cm}
\begin{table}[H]
  \caption{Comparison of <<FORMULA_0096>> distance and KL divergence between target and generated distribution in High Dimensional <<FORMULA_0097>> Opinion Dynamics among Deep GSB, GSBM and FSBM (ours) (Lower is better in all metrics)}
  \label{tab:od-met}
   \centering
...
Subsequently, we consider high-dimensional opinion depolarization, where each particle possesses a high-dimensional opinion <<FORMULA_0098>> that evolves through interaction with the other agents under polarizing dynamics 
% \begin{equation}
...
\textit{ Assume <<FORMULA_0101>>, with <<FORMULA_0102>>, and <<FORMULA_0103>>, and consider a stochastic random variable $X_t$ connecting $X_0$ and $X_1$, whose law is the continuous marginal probability path $p_t$ joining <<FORMULA_0104>>. Starting from the static semi-supervised guided entropic OT problem in \eq{static_SSOT}, we modify it appropriately and obtain the following relaxed dynamic formulation.}
    <<FORMULA_0105>>
...
\\
\textit{Sketch of Proof.} Let <<FORMULA_0106>> be the two given boundary laws, recall the static formulation: 
<<FORMULA_0107>>.
...
Starting from the EOT problem, from <<FORMULA_0109>> and <<FORMULA_0110>>, we can consider any intermediate point $t_1$ and find the optimal paths <<FORMULA_0111>>, <<FORMULA_0112>> between the endpoints in the two newly formed subproblems (i.e., from $t_0$ to $t_1$, and $t_1$  to $t_N$). The union of the optimal subpaths forms the optimal policy of the initial problem, and the converse is also true \cite{villani2009optimal}. This can be extended for any number $N$ of intermediate points. As  <<FORMULA_0113>> , this leads us to to consider ‘local’ versions of optimal transport problems between infinitesimally close distributions. This implies that the local path $p_{t_i, t_{i+1}}$ converges to the continuous marginal $p_t$, for arbitrarily large number of steps in $[0,1]$ \cite{villani2009optimal}. Therefore, in the next step, we express the guidance constraint EOT problem as a sequence of local EOT problems.

<<FORMULA_0114>>

Subsequently, we rewrite the <<FORMULA_0115>> function between $X_{t_i}$ and $X_{t_{i+1}}$, and as a sum of telescopic series with constant reference point $X_0$ from the source distribution. This induces the difference <<FORMULA_0116>>. Although, this term is positive at the optimal path, in order to ensure positivity of this term throughout the optimization process, we upper bound it with its absolute value.

<<FORMULA_0117>>

Finally, we apply the Girsanov theorem in the KL divergence and Ito’s Lemma in the difference <<FORMULA_0118>> introduced in step 3, where the stochastic terms are dropped assuming zero mean Wiener noise. 

Assuming now that the number of intermediate steps <<FORMULA_0119>>, this yields our continuous dynamic formulation 

<<FORMULA_0120>>

    
\begin{proof}
First, let <<FORMULA_0121>> be the cost from the static optimal transport expression, and let <<FORMULA_0122>> be the two given boundary laws.
Recall the static formulation: 
<<FORMULA_0123>>.
Our objective is to derive a dynamic formulation for the static EOT problem with the cost function <<FORMULA_0124>>. 
We can rewrite the divergence <<FORMULA_0125>>, and the quadratic cost <<FORMULA_0126>>, as <<FORMULA_0127>> assuming a gaussian prior measure $\sQ$ \citep{nutz2021introduction}.
More specifically, it holds that <<FORMULA_0128>>, hence <<FORMULA_0129>>.
We rewrite the objective in \eq{starting_point}, as <<FORMULA_0130>>.
...
Let a stochastic random variable path $(X_t)_{0\leq t\leq 1}$ joining <<FORMULA_0132>> to <<FORMULA_0133>>, namely $X_t$ is an interpolation from $X_0$ to $X_1$. This random variable is assumed to be the solution of the following SDE
<<FORMULA_0134>>
...
Now, application of [\cite{villani2009optimal}, Theorem 7.21] suggests that if we consider $p_t$ be the solution to the infimum above, namely a minimizing curve. The endpoints of the random curve $X_t$, whose law admits an optimal transference plan, are an optimal coupling <<FORMULA_0136>>. Then from the definition of the optimal cost and the minimizing property of the $X_t$
<<FORMULA_0137>>
for with <<FORMULA_0138>>, and <<FORMULA_0139>>. 
It generally holds <<FORMULA_0140>>, with the equality being true for the minimizing path. 
Therefore, we can say that 
<<FORMULA_0141>>
This yields the modified version of the dynamic analogue
<<FORMULA_0142>> 

However, note that <<FORMULA_0143>>.
Subsequently, note for any coupling between $p_{t_i}$, and $p_{t_{i+1}}$, it holds that 
<<FORMULA_0144>>
Finally, in order to ensure positivity of the difference <<FORMULA_0145>>, we upper bound with its absolute value, and finally obtain the relaxed dynamic objective, which admits the following formulation 
<<FORMULA_0146>>
For the first term, assuming the Markovian property of the base measure, we apply the Girsanov theorem in the infinitesimally small time-step <<FORMULA_0147>>: 
<<FORMULA_0148>>
...
For discretized random variable $X_{t_i}$ obeying the following discretized SDE: <<FORMULA_0150>>, application of the discrete version of the Ito's Lemma yields
    <<FORMULA_0151>>
    It holds that <<FORMULA_0152>>, since $G$ does not explicitly depend on $t$, and also assume that the Wiener process increment has zero expectation, hence the equation above is rewritten as 
    <<FORMULA_0153>>
    Now, we make use of the fact that <<FORMULA_0154>>, to take the expectation out of the absolute value.
    <<FORMULA_0155>>
    Note that  $G$ is convex, i.e., <<FORMULA_0156>>, thus we conclude that the expression in \eq{sum_of_G} is upper bound by 
    <<FORMULA_0157>>
...
<<FORMULA_0158>>
where <<FORMULA_0159>>. Finally, we conclude the relaxed dynamic formulation is given by
<<FORMULA_0160>>
subjected to the FPE: <<FORMULA_0161>>, and under the boundary constraints: <<FORMULA_0162>>, and <<FORMULA_0163>>.
\end{proof}


\subsection{Proof of Proposition \ref{th:cond.lagrang.}}\label{sec:CL}
\textit{Let the continuous marginal path $p_t$ satisfy the following decomposition <<FORMULA_0164>>. For optimization with respect to the intermediate path, we freeze the joint distribution at the boundaries $\pi_{0,1}$, which results in \eq{lagrangian} being recasted as }
    <<FORMULA_0165>>
\begin{proof}
Initially, let us deal with the dynamic objective in \eq{lagrangian_app}
<<FORMULA_0166>>

The marginal is constructed as a mixture of conditional probability paths <<FORMULA_0167>>.
Now, we sample pairs of <<FORMULA_0168>>, and fix the couplings. This is equivalent to fixing the vector, which induces the couplings. Consequently, fixing the coupling results in recasting \eq{lagrangian_app} as
<<FORMULA_0169>>
where $u_{t|0,1}$ is used to emphasize the drift that corresponds to the conditional probability path, <<FORMULA_0170>>. Additionally, notice that by fixing the coupling, the expectation $\mathbb{E}_{p_{0|t}}$ is dropped.
<<FORMULA_0171>>
...
\textit{The convex conjugate of the Lagrangian in \eq{cond.lagrang.} is defined as the Hamiltonian <<FORMULA_0176>>. 
The optimization with respect to the drift $u_{t|0,1}$ yields <<FORMULA_0177>>, where <<FORMULA_0178>>, and 
<<FORMULA_0179>>
Finally, we obtain the Hamiltonian associated with the Lagrangian in \eq{cond.lagrang.} 
    <<FORMULA_0180>>}
\begin{proof}
...
Given the convexity of \eq{cond.lagrang.}, we compute the Hamiltonian through the subgradient of the absolute value <<FORMULA_0182>>.
Optimizing with respect to the conditioned drift $u_{t|0,1}$ yields the following
    <<FORMULA_0183>>
    where <<FORMULA_0184>> denotes the subgradient. Note that 
<<FORMULA_0185>> 
This essentially means that the inner products <<FORMULA_0186>>, and <<FORMULA_0187>> share the same sign, namely <<FORMULA_0188>>, when <<FORMULA_0189>>, otherwise  <<FORMULA_0190>>. 
<<FORMULA_0191>> 
We define function <<FORMULA_0192>>, to be equal to:
<<FORMULA_0193>>
and substitute it in \eq{Ham.def.app.}, which yields an expression for $u_t$ as a function of the gradient field
<<FORMULA_0194>>
Now, we will provide an expression for the Hamiltonian dependent only on the gradient field $a_t$ and the feedback term <<FORMULA_0195>>. 
<<FORMULA_0196>>
...
\textit{Given the optimal conditional drift <<FORMULA_0198>>, we match the parameterized drift <<FORMULA_0199>>, by minimizing the optimality gap given by our Entropic Lagrangian Bridge Matching objective as
    <<FORMULA_0200>>
    From Proposition \ref{prop:hamiltonian}, we find that the parameterized drift can be expressed as  <<FORMULA_0201>>.
...
\begin{proposition}
For a Lagrangian function <<FORMULA_0203>>, the convex conjugate is given by the Hamiltonian: <<FORMULA_0204>>, dependent on the gradient field $a_t$.
...
<<FORMULA_0211>>
where, the second equation stems from defining the Hamiltonian of the Lagrangian <<FORMULA_0212>>  with respect to the Lagrange multiplier $s_t$ as: 
<<FORMULA_0213>>.
Now, consider the optimality gap between a parameterized gradient field, and an optimal gradient field <<FORMULA_0214>>, which corresponds to a loss function $\gL^\star$. 
We compute the optimality gap as
<<FORMULA_0215>>
At this point, we define the gradient field function <<FORMULA_0216>>, which is equal to <<FORMULA_0217>>, which finally yields
<<FORMULA_0218>>
where the first equality stems from the fact that at the optimal point (<<FORMULA_0219>>), it holds that <<FORMULA_0220>> \citep{neklyudov2023action, boyd2004convex}. 
This allows us to substitute the expression in the last line of \eq{gap}, <<FORMULA_0221>> with <<FORMULA_0222>>, and express the variational gap as the Bregman Divergence, which emerges by the definition in the last equality finishing the proof of the proposition.
\end{proof}
Therefore, to minimize the optimality gap, we sample sufficiently many uncoupled pairs <<FORMULA_0223>>, and parameterize the gradient field <<FORMULA_0224>>, and try to match the parameterized gradient field on the optimized drift <<FORMULA_0225>>. We express our matching objective an expectation over the sampled pairs of optimality gap derived from the previous proposition
<<FORMULA_0226>>

Substituting for the expressions we have for the functions <<FORMULA_0227>>, we obtain
<<FORMULA_0228>> 
At this point, we take cases for the different values of $g_t$. 
\textbf{First: } <<FORMULA_0229>>, and <<FORMULA_0230>>:
<<FORMULA_0231>>
\textbf{Second: } <<FORMULA_0232>>, and <<FORMULA_0233>>:
<<FORMULA_0234>>
\textbf{Third: }<<FORMULA_0235>>, , and <<FORMULA_0236>> , with <<FORMULA_0237>>:
<<FORMULA_0238>>
where the second equality comes from the fact that we added the quantity <<FORMULA_0239>>. 
Finally, we can succinctly express the optimality gap combining \eq{OG1}, and \ref{eq:OG2}:
<<FORMULA_0240>>
But notice that by the definition of <<FORMULA_0241>> in \eq{drift_grad.field}, we have <<FORMULA_0242>>, hence the minimization problem of the variational gap in \eq{Breg.Div} is expressed as
<<FORMULA_0243>>
\end{proof}
...
<<FORMULA_0245>>
,where <<FORMULA_0246>> is the interpolant function between the pinned endpoints, and <<FORMULA_0247>> is the Brownian motion. 
...
<<FORMULA_0249>>
where $\nu$ is given by the definition of <<FORMULA_0250>>. We can see that substitution of <<FORMULA_0251>> and <<FORMULA_0252>>, to the conditional drift expression indeed yields the desired drift of the Brownian bridge <<FORMULA_0253>>.


\textbf{Spline optimization.}
Efficient optimization is achieved by parameterizing <<FORMULA_0254>> respectively as $d$- and $1$-D splines using several knots (i.e., control points) sampled sparsely and uniformly along the time steps <<FORMULA_0255>>: <<FORMULA_0256>> and <<FORMULA_0257>>. Among those knots, we fit the time-dependent interpolant function $I_t$, and the standard deviation $\sigma_t$
<<FORMULA_0258>>
%
...
The number of control points $K$ is much smaller than the discretization steps (<<FORMULA_0259>>30 for all experiments).
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
    \STATE {\bfseries Input}: <<FORMULA_0260>> where <<FORMULA_0261>>, and the pairs from the key-point (KP) set <<FORMULA_0262>>, and obtain their trajectories.
         % \Require $x_0$, $x_1$, $\{X_{t_k}\}$
    \STATE Initialize $I_t$, $\sigma_t$
    \STATE {\bfseries repeat}
        \STATE \hskip1em  Sample <<FORMULA_0263>>  
        \STATE \hskip1em  Evaluate the objective <<FORMULA_0264>> using the KP pairs
        \STATE \hskip1em  Estimate the conditional drift <<FORMULA_0265>> \eq{cond.drift}
        \STATE \hskip1em  Take gradient step w.r.t control pts. <<FORMULA_0266>>
    \STATE {\bfseries until} converges
...
This is attributed to the utilization of independent samples from $p_{t|0,1}$, which are known in closed form. Finally, recall that since we solve \eq{cond.lagrang.} for each pair <<FORMULA_0267>> and later marginalize to construct $p_t$.
...
<<FORMULA_0268>>
where <<FORMULA_0269>> is the guidance regularization hyperparameter responsible to amplify $G$, and bypass the minimization of the kinetic energy of the non-paired samples enough to satisfy the feasibility of the solution.
where for our experiments, the selected distance between the non-coupled samples and the fixed KP sample was the $L_1$ norm  <<FORMULA_0270>>. The <<FORMULA_0271>> aligned data point was the closest keypoint sample to non-aligned particle $X_t$ at time <<FORMULA_0272>>. To compute the guidance function, we need the trajectories of the KP samples. These are assumed to be given only through their samples and fixed for each KP pair. In practice in the next sections, we discuss how we acquired those trajectories for each of our experiments.
...
More explicitly, the two compared frameworks were trained on transporting the samples <<FORMULA_0273>> to <<FORMULA_0274>>. In the evaluation phase, the task is always to transport the samples to the same target distribution as in the training phase, however, the initial conditions change throughout the different tasks. 
For the first task (Vanilla), we draw new samples from the same distribution <<FORMULA_0275>>, for the second task we shift the mean of the distribution, for the third task we increase the standard deviation, and in the fourth task we sample from a uniform distribution.
The initial conditions for each task are the following: 
\begin{itemize}
    \item Vanilla: <<FORMULA_0276>>
    \item Perturbed Mean: <<FORMULA_0277>>
    \item Perturbed STD: <<FORMULA_0278>>
    \item Uniform Distribution with geometric mean $[-11, -1]$
\end{itemize}
where <<FORMULA_0279>> represents a Gaussian with mean the point <<FORMULA_0280>>, and standard deviation equal to $\sigma$. 
...
Subsequently, we revisit the high-dimensional opinion depolarization of a population. Each particle possesses a high-dimensional opinion <<FORMULA_0281>> that evolves under polarizing dynamics 
<<FORMULA_0282>>
...
At each time step $t$,  all agents receive the same random information <<FORMULA_0283>> sampled independently of $p_t$, then react to this information according to
 
<<FORMULA_0284>>
where <<FORMULA_0285>> are opinions and  <<FORMULA_0286>> and <<FORMULA_0287>> is the agreement function indicating whether the two opinions agree on the information $\xi_t$.
...
\begin{table}[H]
  \caption{<<FORMULA_0288>> values between input and generated images in 4 translation tasks for DSBM, LOSBM, and our FSBM (Lower is better). Note the values are divided by $2^20$, which is the total number of pixels to get a mean $L2$-norm over all the pixels}
  \label{tab:l2}
   \centering
  \begin{tabular}{llllll}
  \toprule

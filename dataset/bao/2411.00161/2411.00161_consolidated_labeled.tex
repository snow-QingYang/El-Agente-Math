...
\unanchoredfootnote{<<FORMULA_0001>>Correspondence to \texttt{wyrwal.kacper@gmail.com} and \texttt{viacheslav.borovitskiy@gmail.com}.\newline\hspace*{1em} Code available at \url{https://github.com/KacperWyrwal/residual-deep-gps}.}
...
\draw [rounded corners] (rect-nw) rectangle node[above, yshift=1.53cm] {<<FORMULA_0002>>} (rect-se);

% Draw arrows
\draw [arrow] ([xshift=-15pt]img1.east) -- node[above, yshift=1pt, xshift=-1pt] {$\mathrm{GVF}$}([xshift=3pt]img2.west);
\draw [arrow] ([xshift=-15pt]img2.east) -- node[above] {<<FORMULA_0003>>} ([xshift=3pt]img3.west);
...
For such $f$ there always exist a mean <<FORMULA_0004>> on the input domain $X$ of $f$ and a kernel <<FORMULA_0005>> such that <<FORMULA_0006>> for all <<FORMULA_0007>>.
In this case, we write <<FORMULA_0008>>.

GPs define useful priors for learning functions from noisy observations <<FORMULA_0009>> at given input locations <<FORMULA_0010>> within the Bayesian framework.
In fact, if the observation likelihood is assumed Gaussian <<FORMULA_0011>>, then the posterior is a GP \cite{rasmussen2006} with
<<FORMULA_0012>>

The input domain $X$ can, in principle, be any set.
However, a good kernel <<FORMULA_0013>> is necessary to define practical GP models on $X$.
If <<FORMULA_0014>>, the most widely used kernels are from the Matérn family \cite{rasmussen2006}, including, as a limiting case, the especially popular squared exponential kernel.\footnote{This kernel has many names; it is also known as the RBF, Gaussian, heat, or diffusion kernel.}
...
<<FORMULA_0015>>
where <<FORMULA_0016>> are the eigenpairs of the Laplace--Beltrami operator on $X$, $d$ is the dimension of $X$, and $C_{\nu, \kappa}$ is a normalisation constant ensuring <<FORMULA_0017>>.
The infinite sum must be truncated for computational tractability; nevertheless, the rapid decay of the coefficients <<FORMULA_0018>> makes this a sensible approximation with convergence guarantees \cite{rosa2023}.
...
A deep Gaussian process $F$ is a composition <<FORMULA_0019>> of multiple shallow GPs $f^l$ \cite{damianou2013}.
To allow for richer structure, the hidden \emph{layers} $f^l$, for <<FORMULA_0020>>, are typically \emph{vector-valued} GPs <<FORMULA_0021>>, i.e. vectors of scalar-valued GPs stacked together and potentially correlated with each other \cite{alvarez2012}.
The resulting random function $F$ is itself not a GP.
Thus, even for Gaussian likelihoods <<FORMULA_0022>>, inference for <<FORMULA_0023>> is intractable.
...
In it, the intractable posterior <<FORMULA_0024>> is approximated in terms of the KL divergence metric $\KL$ by the elements of a certain \emph{variational family} of tractable distributions.
...
Take some <<FORMULA_0025>>.
A sparse GP $f_{\v{z}, \v{m}, \v{S}}$ is a family of GPs parameterised by a set of $m$ \emph{inducing locations} <<FORMULA_0026>>, as well as a mean vector <<FORMULA_0027>> and a covariance matrix <<FORMULA_0028>> which determine the corresponding \emph{inducing variable} distribution <<FORMULA_0029>>.
Specifically,
<<FORMULA_0030>>
where <<FORMULA_0031>> is the prior $f$ conditioned on <<FORMULA_0032>>.
Intuitively, $\v{z}$ are the pseudo-inputs, $\v{u}$ are random pseudo-observations, and <<FORMULA_0033>> is a kind of pseudo-posterior.
It is a GP with
<<FORMULA_0034>>
These can be readily generalised to the vector-valued setting, with <<FORMULA_0035>> and <<FORMULA_0036>>.

For a deep GP, the respective variational family is the distribution of a composition of sparse GPs:
<<FORMULA_0037>>
The variational parameters <<FORMULA_0038>> are found by minimising <<FORMULA_0039>>, which is equivalent \cite{salimbeni2017} to maximising the following evidence lower bound (ELBO)
<<FORMULA_0040>>
where <<FORMULA_0041>> and <<FORMULA_0042>>.
The second term can be computed exactly using the formula for $\KL$ between two Gaussian vectors.
The first term is intractable, but can be efficiently approximated by drawing a sample from <<FORMULA_0043>>---this can be done in a layerwise fashion, as \textcite{salimbeni2017} suggest---and subsampling the sum over a mini-batch of inputs $\v{x}$.
...
While building a GP with inputs in $X$ amounts to finding an appropriate kernel <<FORMULA_0045>>, handling outputs in $X$ requires redefining the inherently Euclidean notion of a \emph{Gaussian}.
...
There, each layer <<FORMULA_0046>> is of the form
<<FORMULA_0047>>
where $g^l$ is a zero-mean GP.
That is, each layer
$f^l$ displaces its input $x$ by a \emph{residual} vector <<FORMULA_0048>>, the difference to the identity transform, much like a residual connection in neural networks \cite{he2016}, which is modelled~by~a~GP.
On a manifold <<FORMULA_0049>>, when <<FORMULA_0050>> and <<FORMULA_0051>>, the addition operation in <<FORMULA_0052>> is undefined.
However, there is a natural generalisation:
<<FORMULA_0053>>
Here, <<FORMULA_0054>> is the \emph{exponential map}, the canonical mapping of the \emph{tangent space} <<FORMULA_0055>> at <<FORMULA_0056>>---i.e., the linear space of vectors tangent to $X$ at point $x$---back to $X$ itself.
That is, a point $x$ is still displaced by the vector <<FORMULA_0057>>, but in a geometrically sound manner.
...
Still, a major technical difficulty remains: for different inputs $x$, the value <<FORMULA_0058>> must lie in the different spaces <<FORMULA_0059>>.
...
A vector field on a manifold $X$ is a function that takes each <<FORMULA_0060>> to an element of the tangent space <<FORMULA_0061>> of $X$ at the point $x$.
If $X$ is\footnote{\emph{Nash embedding theorem} proves this is always the case for a large enough ambient space dimension <<FORMULA_0062>>.} a submanifold of $\R^D$---like the $2$-sphere is a submanifold of $\R^3$---the difference between a vector field and a general vector-valued function on $X$ is very intuitive: in the latter, a vector attached to a point <<FORMULA_0063>> can point in any direction, while in the former it must always be tangential to the manifold $X$ at $x$.
...
\textcite{hutchinson2021} propose a simple idea, to build a GVF $g$ from any given vector-valued GP <<FORMULA_0064>> by \emph{projecting} its outputs onto the appropriate tangent spaces.
Such a projection <<FORMULA_0065>> exists because, if $X$ is a submanifold of $\R^D$, then any tangent space <<FORMULA_0066>> can be identified with a linear subspace of $\R^D$.
Thus, <<FORMULA_0067>> defines a random vector field (see \Cref{fig:projected_gvf}), which turns out to be Gaussian because of the linearity of $P_{(\cdot)}$.


\paragraph{Coordinate-frame-based GVFs}
Given any \emph{coordinate frame} <<FORMULA_0068>>---that is, a set of functions such that <<FORMULA_0069>> is a linear basis of <<FORMULA_0070>>---and a vector-valued GP <<FORMULA_0071>> with components <<FORMULA_0072>>, the equation <<FORMULA_0073>> defines a GVF.
...
<<FORMULA_0074>>
Here, $s_j$ are the eigenfields of the Hodge Laplacian on $X$ that correspond to the eigenvalues <<FORMULA_0075>>, $\otimes$~is the tensor product, and $\Phi_{\nu,\kappa}$ is exactly as in~\Cref{eq:matern_kernel_eigenvalues_sum}.
...
Consider the analogue of the variational family <<FORMULA_0076>> in~\Cref{eqn:eucl_deep_gp_vf} with the sparse GP layers $f_{\v{z}^l, \v{m}^l, \m{S}^l}$ replaced by
<<FORMULA_0077>>
...
For $g_{\v{z}^l, \v{m}^l, \m{S}^l}$, however, <<FORMULA_0078>> with inducing locations <<FORMULA_0079>>, and
<<FORMULA_0080>>
where $\operatorname{im}$ denotes the \emph{image} of a linear operator, and the last two constraints ensure that the random pseudo-observations <<FORMULA_0081>> are tangent vectors which lie in the appropriate tangent spaces.

To satisfy the aforementioned constraints during optimisation, one can represent
<<FORMULA_0082>>
with arbitrary <<FORMULA_0083>>, arbitrary positive semi-definite <<FORMULA_0084>>, and <<FORMULA_0085>> denoting the projection of vectors in $\R^D$ onto the tangent space <<FORMULA_0086>>, as discussed in~\Cref{sec:gaussian-vector-fields}.
...
Finally, to approximate ELBO, we need to sample <<FORMULA_0087>>.
As in~\textcite{salimbeni2017}, this can be done sequentially.
Specifically, if <<FORMULA_0088>> denotes the desired sample, then
<<FORMULA_0089>>
and, given $\hat{f}^{l-1}_i$, each individual <<FORMULA_0090>> can be sampled in the usual manner.
...
It is constructed by replacing the inducing locations $\v{z}$ by \emph{inducing linear functionals} <<FORMULA_0091>>.
Each $\zeta_{j}$ takes in a vector field $g$ and outputs a real number.
These <<FORMULA_0092>> define a sparse GVF through
<<FORMULA_0093>>
where <<FORMULA_0094>> is the prior $g$ conditioned on <<FORMULA_0095>>, where <<FORMULA_0096>>.
For example, linear functionals of the form <<FORMULA_0097>>---here, <<FORMULA_0098>> is a coordinate frame---can be used to recover the usual doubly stochastic variational inference considered above.

The mean and covariance of $g_{\v{\zeta}, \v{m}, \v{S}}$ are given by \Cref{eq:variational_posterior_mean_and_kernel}, with $\v{z}$ replaced by <<FORMULA_0099>> and
<<FORMULA_0100>>
see, for example, \textcite{lazarogredilla2009, vanderwilk2020}.

Now, if the kernel of $g$ can be expressed as <<FORMULA_0101>> where <<FORMULA_0102>> is an orthonormal basis---this is obviously so for Hodge GVFs, but is also often the case for other GVFs on compact manifolds---the inducing functionals <<FORMULA_0103>> yield very simple covariance matrices
<<FORMULA_0104>>
In particular, <<FORMULA_0105>> is diagonal, making it trivial to invert.
...
Expectations <<FORMULA_0106>> and variances <<FORMULA_0107>> of the approximate posterior $F_{\v{\theta}}$ cannot be computed exactly.
Instead, they can be estimated by appropriate Monte Carlo averages, with~\Cref{eqn:sequential_sampling} providing a way to sample <<FORMULA_0108>>.
However, since these estimates ignore the correlation between <<FORMULA_0109>> and <<FORMULA_0110>>, they are not continuous as functions of~$x$.
When continuity or differentiability of <<FORMULA_0111>> and <<FORMULA_0112>> are desirable, another method can be used.
The key idea in this case is to draw (approximate) samples from <<FORMULA_0113>> which happen to be actual functions, for example linear combinations of some analytic basis functions or compositions of such.
...
We take <<FORMULA_0114>> training inputs $\v{x}$ on a Fibonacci lattice on $\mathbb{S}_2$ and put <<FORMULA_0115>>, <<FORMULA_0116>>.
...
All runs are conducted $5$ times with different random seeds for the observation noise <<FORMULA_0117>>.

\paragraph{Models}
The baseline is a deep GP with Euclidean (rather than manifold-to-manifold) layers.
It is constructed by composing a vector-valued Matérn GP whose signature is <<FORMULA_0118>> with the Euclidean deep GP of \textcite{salimbeni2017} on $\R^3$.
...
Specifically, they reported that approximate variational inference using the shallow analogue of the interdomain inducing variables applied to data mapped\footnote{Mapping <<FORMULA_0119>>, where <<FORMULA_0120>> is a \emph{bias} term, <<FORMULA_0121>>.} from $\R^d$ to the proxy manifold of $\mathbb{S}_{d}$ can be significantly faster than inducing-location-based variational inference for a Euclidean GP on the original data, while achieving competitive predictive performance.
...
The addition theorem gives a relation between all spherical harmonics  corresponding to the <<FORMULA_0122>>-th largest eigenvalue of the negative Laplace--Beltrami operator <<FORMULA_0123>>, denoted <<FORMULA_0124>>, and Gegenbauer polynomials $C_k^{(\alpha)}$, other special functions:
<<FORMULA_0125>>
with the dot product computed after embedding $\mathbb{S}_d$ in $\mathbb{R}^{d+1}$ as the unit sphere centred at the origin, <<FORMULA_0126>>, and $c_{k, d}$ some known absolute constants.
Thus, when computing the scalar Matérn kernel on $\mathbb{S}_d$, we truncate the infinite sum in \Cref{eq:matern_kernel_eigenvalues_sum} to include all spherical harmonics up to the <<FORMULA_0127>>-th eigenvalue, and apply \Cref{eq:addition_theorem}.
This gives the following formula
<<FORMULA_0128>>
In case of the Hodge Matérn kernel on $\mathbb{S}_2$, we also apply addition theorem, only noting that here $\phi_{k, j}$ are replaced with the normalised vector spherical harmonics: <<FORMULA_0129>> for the divergence-only kernel, and <<FORMULA_0130>> for the curl-only kernel, where $\star$ is the Hodge star operator \cite{robertnicoud2024}.
In this case, <<FORMULA_0131>>.
Thus,
<<FORMULA_0132>>

The Hodge-compositional kernel, which we typically use, is the sum <<FORMULA_0133>>.

\paragraph{Accelerated training with whitened inducing variables}
In all the models we test, approximate inference requires a variational mean vector $\v{m}$ and a variational covariance matrix $\m{S}$ that parameterise <<FORMULA_0134>>.\footnote{$\m{S}$ is parametrised by its lower Cholesky factor to ensure positive definiteness during optimisation.}
However, to accelerate convergence during training, instead of working with the inducing variables $\v{u}$ directly, we work with \emph{whitened} inducing variables <<FORMULA_0135>>, where $\m{L}$ is the lower Cholesky factor of <<FORMULA_0136>> or <<FORMULA_0137>> \cite{matthews2016}.
Thus, in practice, denoting the whitened variational mean and covariance of <<FORMULA_0138>> as $\v{m}'$ and $\m{S}'$, we use a modified version of~\Cref{eq:variational_posterior_mean_and_kernel}:
<<FORMULA_0139>>

\subsection{Models}
\paragraph{Mean and kernel}
In all models, we equip constituent GPs with a zero mean and an appropriate variant of the Matérn kernel, initialised with length scale <<FORMULA_0140>>.
For kernels of output layers we initialise the variance to <<FORMULA_0141>>, while for kernels in hidden layers of an $L$-layer deep GPs we set <<FORMULA_0142>> at the start of training.
In \Cref{subsection:synthetic_examples}, \Cref{subsection:euclidean_acceleration}, and \Cref{subsection:wind_velocity_modelling_on_the_globe} we initialise the smoothness parameter to <<FORMULA_0143>>, while in \Cref{subsection:geometry_aware_bayesian_optimisation} we set it to <<FORMULA_0144>> to replicate the setup in \textcite{jaquier2022}.
...
Wherever we employ Hodge GVFs we use the Hodge compositional kernel, with separate <<FORMULA_0145>> for the curl-free and divergence-free parts.
...
The solid lines represent the mean, while the shaded areas represent the <<FORMULA_0146>> region around it, where $\sigma$ is the standard deviation, and all statistics were computed over $5$ randomised runs.}
...
We define the benchmark function <<FORMULA_0148>> by
<<FORMULA_0149>>
where
<<FORMULA_0150>>
with $x$ being an element of $\mathbb{S}_2$ embedded into $\mathbb{R}^3$ as the unit sphere centred at the origin.
<<FORMULA_0151>> are spherical harmonics, smooth functions of their parameters.
Singularities in <<FORMULA_0152>> are caused by the composition with $\varphi$, which converts $x$ from Cartesian to spherical coordinates, but swaps the positions of the colatitude $\theta$ and longitude $\phi$.
The function <<FORMULA_0153>> has singularities at the poles, while the function <<FORMULA_0154>> has singularities around the equator.
...
Different subplots correspond to different training set sizes $N$.
The solid lines represent the mean, while the shaded areas represent the <<FORMULA_0156>> region around it, where $\sigma$ is the standard deviation; all statistics are computed over $10$ randomised runs.
}
...
Different subplots correspond to different training set sizes $N$.
The solid lines represent the mean, while the shaded areas represent the <<FORMULA_0157>> region around it, where $\sigma$ is the standard deviation; all statistics are computed over $10$ randomised runs.
}
...
\end{figure}
\paragraph{Data}
To obtain an irregular function <<FORMULA_0158>> on $\mathbb{S}_2$, we modify the target function from \Cref{subsection:synthetic_examples} to have only one global minimum near a singularity point.
Specifically, <<FORMULA_0159>> was defined by
<<FORMULA_0160>>
...
To evaluate the models, we compute the MSE and NLPD via Monte Carlo sampling as described in \Cref{eqn:sequential_sampling}, we visualise the predictive uncertainty at a point $x_i$ which is computed as <<FORMULA_0161>>, where $\m{S}^n_i$ is the posterior covariance matrix of the last layer given the $n$-th sample from the penultimate layer, and <<FORMULA_0162>> is the Frobenius norm.
...
\begin{figure}
    ...
    Residual deep GPs had their inputs mapped from $\mathbb{R}^d$ to $\mathbb{S}_{d}$ via <<FORMULA_0163>>.
...
A potential reason for this phenomenon may be identified by examining the kernel of a posterior sparse Matérn GP <<FORMULA_0164>>---more specifically, its whitened reparameterisation (see \Cref{app:implementation}), as that makes equations cleaner.
In this, it will be helpful to define
<<FORMULA_0165>>
which allows us to express the Matérn kernel approximated with <<FORMULA_0166>> eigenfunctions as <<FORMULA_0167>>.
...
<<FORMULA_0169>>
For <<FORMULA_0170>>, posterior covariance reduces to the second term only, which is determined by the kernel hyperparameters and the variational covariance matrix.
However, for <<FORMULA_0171>>, the first term contributes additional variance that can only be reduced by changing the hyperparameters of the prior, like length scale and prior variance, rather than the variational parameters $\v{m}'$ and $\m{S}'$.
...
In practice, we observe that this can lead to a difficulty if the <<FORMULA_0172>> is already at the desired variance but $\m{S'}$ must still be adjusted to approximate the true covariance, while <<FORMULA_0173>> is still too large.
In this case, the first mechanism pushes <<FORMULA_0174>> downwards by lowering the prior variance $\sigma^2$, which necessarily also reduces <<FORMULA_0175>>.
Consequently, $\m{S}'$ must increase to compensate for this.
This process results in a tug-of-war between the variational parameters and kernel hyperparameters, which seems to make optimisation difficult, and is thus a possible reason for the drop in performance as <<FORMULA_0176>>.

One remedy is to set <<FORMULA_0177>>, which is what we actually do in our experiments.
...
Our extension expands the $\m{S}'$ matrix with a parametrised diagonal $\m{D}'$ corresponding to the <<FORMULA_0178>> eigenfunctions not previously used in the variational family but used in the kernel, giving
<<FORMULA_0179>>
This eliminates the aforementioned conflict, allowing the variational parameters to affect both terms in a similar way.

Ostensibly, when <<FORMULA_0180>> is large, it significantly impacts performance.
...
With this method, the number of additional variational parameters is minimal, and, comparing our extended variational family with its original variant, there is practically no increase in computation time, as <<FORMULA_0181>> is simply substituted for <<FORMULA_0182>>.

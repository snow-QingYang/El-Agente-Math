...
def main() -> None:
    ...
    st.sidebar.markdown(f"**Total Reports:** {metadata.get('total_reports', 'N/A')}")
    st.sidebar.markdown(f"**Valid Comparisons:** {metadata.get('valid_comparisons', 'N/A')}")
    st.sidebar.markdown(f"**Matches:** {metadata.get('matches', 'N/A')}")
    st.sidebar.markdown(f"**Mismatches:** {metadata.get('mismatches', 'N/A')}")
    st.sidebar.markdown(f"**Match Rate:** {metadata.get('match_rate', 'N/A')}%")
    st.sidebar.markdown(f"**Errors:** {metadata.get('errors', 'N/A')}")
    st.sidebar.markdown(f"**Skipped:** {metadata.get('skipped', 'N/A')}")

    st.sidebar.divider()
    st.sidebar.header("Filter")
    filter_option = st.sidebar.radio(
        "Show",
        ["All", "Matched", "Mismatched", "Errors", "Skipped"],
        index=0,
    )
    ...
    def filter_entries(entries: list[dict]) -> list[dict]:
        ...
        if filter_option == "Mismatched":
            return [e for e in entries if e.get("status") == "success" and e.get("matched") is False]

    ...
    while i < len(words):
        ...
        if text.endswith("-") and i + 1 < len(words):
            ...
            if abs(w["top"] - nxt["top"]) < 3:
                ...
                i += 2
                continue
        merged.append({"text": text, "bbox": [w["x0"], w["top"], w["x1"], w["bottom"]]})
        i += 1
    return merged
    ...
    for w in words:
        ...
        if w["x1"] <= target["x0"]:
            if left is None or w["x1"] > left["x1"]:
                left = w
        elif w["x0"] >= target["x1"] and (right is None or w["x0"] < right["x0"]):
            right = w
    if left and is_numeric_token(left.get("text", "")) and target["x0"] - left["x1"] <= max_gap:
        return True
    return bool(
        right and is_numeric_token(right.get("text", "")) and right["x0"] - target["x1"] <= max_gap
    )
...
def find_phrase_bbox(page, phrase: str, left_cutoff):
    ...
    if is_numeric:
        ...
        for w in words:
            if normalize_word(w.get("text", "")) == phrase and not has_adjacent_numeric(words, w):
                return [w["x0"], w["top"], w["x1"], w["bottom"]]
    else:
...
def find_anchor_pattern(md_text: str, lines, prefer_head):
    ...
    for strict in (True, False):
        for line in candidate_lines:
            ...
            candidate_text = line[:ANCHOR_WINDOW] if prefer_head else line[-ANCHOR_WINDOW:]
            ...
            for count in ANCHOR_WORD_COUNTS:
                ...
                if not pattern:
                    continue
                if re.search(pattern, md_text, flags=re.MULTILINE | re.DOTALL | re.IGNORECASE):
                    return pattern, line
    return "", ""
...
def resolve_line_context(pdf, line_str, content_list):
    for page_idx, page in enumerate(pdf.pages):
        ...
        if not line_matches:
            ...
            if alt_matches:
                alt_matches.sort(key=lambda b: abs((b.get("_page_idx") or 0) - page_idx))
        ...
        below_block = (
            page_text_blocks[anchor_idx + 1] if anchor_idx + 1 < len(page_text_blocks) else None
        )
...
def locate_block_from_pdf(base_dir: Path, line_spec: str, output_path: Path) -> bool:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        if is_range and same_anchor:
            ...
            start_anchor = (
                find_nearest_anchor_block(start_page_blocks, start_anchor_idx - 1, -1)
                or start_ctx["anchor"]
            )
            end_anchor = (
                find_nearest_anchor_block(end_page_blocks, end_anchor_idx + 1, 1)
                or end_ctx["anchor"]
            )
            if start_anchor is None:
                start_anchor = find_prev_anchor_block_content(content_list, start_content_page)
            if end_anchor is None:
                end_anchor = find_next_anchor_block_content(content_list, end_content_page)
            ...
            if line_override:
                ...
                if start_anchor is None:
                    start_anchor = find_prev_anchor_block_content(content_list, start_content_page)
                if end_anchor is None:
                    end_anchor = find_next_anchor_block_content(content_list, start_content_page)
                start_lines_override = None
                end_lines_override = None

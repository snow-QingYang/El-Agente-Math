...
def save_json(content, destination: Path):
...
def download_zip(download_urls, destination: Path):
...
def unzip_results(zip_path: Path, destination: Path) -> None:
    with zipfile.ZipFile(zip_path, "r") as archive:
        archive.extractall(destination)


def parse_pdf(pdf_path: Path, output_root: Path) -> Path:
    ...
    try:
        ...
        raw_response_path.write_text(json.dumps(status_response, indent=2), encoding="utf-8")
